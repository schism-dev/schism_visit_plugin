...
SCHISMAtt::SCHISMAtt(const std::string& a_attName)
    : m_name(a_attName),
      m_num_str_value(0),
      m_num_float_value(0),
      m_num_int_value(0),
...
std::string SCHISMAtt::name() const { return m_name; }
...
bool SCHISMAtt::is_valid() const { return m_is_valid; }
...
SCHISMDim::SCHISMDim() : m_is_valid(false), m_name("junkdim"), m_size(0) {}

SCHISMDim::~SCHISMDim() {}

bool SCHISMDim::is_valid() const { return m_is_valid; }

int SCHISMDim::size() const { return m_size; }

int SCHISMDim::id() const { return m_id; }

std::string SCHISMDim::name() const { return m_name; }

SCHISMVar::SCHISMVar(const std::string& a_varName)
    : m_is_valid(true),
      m_data_cached(false),
      m_name(a_varName),
...
int SCHISMVar::num_dims() const { return m_num_dim; }
...
std::string SCHISMVar::name() const { return m_name; }
...
bool SCHISMVar::is_valid() const { return m_is_valid; }

bool SCHISMVar::put_a_float(const float& a_value, int* a_dim1Loc) {
    return false;
}

bool SCHISMVar::get(int* a_buffer) const { return false; }

bool SCHISMVar::get(float* a_buffer) const { return false; }

void SCHISMVar::set_cur(const int& a_TIMERecord) { return; }
...
int SCHISMVar::num_component() const { return m_num_component; }

std::string SCHISMVar::get_horizontal_center() const {
    return m_horizontal_center;
}

std::string SCHISMVar::get_vertical_center() const { return m_vertical_center; }
...
SCHISMFile::SCHISMFile(const std::string a_SCHISMOutputFile)
    : m_total_num_vars(0),
      m_total_num_dims(0),
...
std::string SCHISMFile::level_center() const { return m_layer_center; }

std::string SCHISMFile::data_center() const { return m_data_center; }
...
std::string SCHISMFile::file() const { return m_SCHISMOutputFile; }
bool        SCHISMFile::is_valid() const { return m_is_valid; }

int SCHISMFile::num_vars() const { return m_total_num_vars; }

int SCHISMFile::num_dims() const { return m_total_num_dims; }
...
SCHISMDim* SCHISMFile::get_invalidDim() const { return new SCHISMDim(); }
...
// empty api
bool SCHISMFile::read(char* a_buffer, const int& a_numByte) { return false; }

// empty api
void SCHISMFile::set_cur(const int& a_TIMEStep, const int& a_extraOffset) {
    return;
}
