...
int avtSCHISMFileFormatImpl::GetNTimesteps(const std::string& a_filename) {
...
void avtSCHISMFileFormatImpl::FreeUpResources(void) {
    if (m_time_ptr) {
        delete m_time_ptr;
    }
...
void avtSCHISMFileFormatImpl::PopulateDatabaseMetaData(
    ...
    if (m_data_center != MeshConstants::ELEM) {
        ...
        a_avtFile->addScalarVarToMetaData(a_metaData, m_node_surface_label,
                                          mesh, nodeCent);
...

{
    ...
    // vector data
        ...
        a_avtFile->addScalarVarToMetaData(a_metaData, MeshConstants::LEVEL,
                                          mesh3d, faceCent);
    } else {
    }
}
void avtSCHISMFileFormatImpl::addNodeCenterData(avtDatabaseMetaData* a_metaData,
    ...
    //  scalar var having layer dim
        ...
        // also add bottom, surface and depth average state option
        a_avtFile->addScalarVarToMetaData(
            a_metaData, label + m_surface_state_suffix, m_mesh_2d, avtCenter);
        debug1 << "add  " << label + m_surface_state_suffix << " ";
        a_avtFile->addScalarVarToMetaData(
            a_metaData, label + m_bottom_state_suffix, m_mesh_2d, avtCenter);
        debug1 << "add  " << label + m_bottom_state_suffix << " ";
        a_avtFile->addScalarVarToMetaData(
            a_metaData, label + m_depth_average_suffix, m_mesh_2d, avtCenter);
        debug1 << "add  " << label + m_depth_average_suffix << " ";
...
void avtSCHISMFileFormatImpl::PopulateStateMetaData(
    avtDatabaseMetaData* a_metaData,
    avtSCHISMFileFormat* a_avtFile,
    int                  a_timeState) {
...
void avtSCHISMFileFormatImpl::create2DUnstructuredMesh(
    vtkUnstructuredGrid* a_uGrid, int* a_meshEle, const int& a_timeState) {
...
void avtSCHISMFileFormatImpl::create3DUnstructuredMesh(
    ...
    for (int iLayer = 0; iLayer < m_num_layers - 1; iLayer++) {
        ...
        {
            nodePtrTemp =
                a_meshEle + (MeshConstants::MAX_NUM_NODE_PER_CELL + 1) * iCell;
            int numberOfNodeInCell = *nodePtrTemp;
            ...
            if (numberOfNodeInCell == 3) {
                ...
                insertQuad3DCell(a_uGrid, validTopNodeNum, validBottomNodeNum,
                                 validTopNode, validBottomNode, nodePtrTemp,
                                 a_2DPointto3DPoints, iCell, iLayer);
                // nodePtrTemp += (MeshConstants::MAX_NUM_NODE_PER_CELL+1);
            }
...
void avtSCHISMFileFormatImpl::create3DPointFaceMesh(
    vtkUnstructuredGrid* a_uGrid, int* a_meshEle, const int& a_timeState) {
    int numNodes =
...
vtkDataSet* avtSCHISMFileFormatImpl::GetMesh(int                  a_timeState,
                                             avtSCHISMFileFormat* a_avtFile,
...
void avtSCHISMFileFormatImpl::insertTriangle3DCell(
    vtkUnstructuredGrid* a_uGrid,
...
void avtSCHISMFileFormatImpl::insert8NodesPolyhedron(
    vtkUnstructuredGrid* a_uGrid,
    vtkIdType*           a_verts,
    int*                 a_validTopNode,
    int*                 a_validBottomNode,
...

{
    ...
    {
        debug1 << "pyramid at layer " << a_layerID << " cell " << a_Cell << " "
               << topCoplane << "\n";
        ;
        vtkIdType verts[5];
...
void avtSCHISMFileFormatImpl::loadAndCacheZSide(const int& a_timeState,
...
void avtSCHISMFileFormatImpl::loadAndCacheZEle(const int& a_timeState,
...
void avtSCHISMFileFormatImpl::loadAndCacheZ(const int& a_timeState,
                                            float*     a_nodeZPtr) {
...
void avtSCHISMFileFormatImpl::getMeshDimensions(
    MeshProvider* a_meshProviderPtr) {
...
void avtSCHISMFileFormatImpl::loadMeshCoordinates(
    MeshProvider* a_meshProviderPtr) {
...
vtkDataArray* avtSCHISMFileFormatImpl::GetVar(int         a_timeState,
    ...
    float* valBuff;
    int    numTotalLayers =
        m_num_layers; // there is no subtract for prismcenter
                      // data for prism center also maintain
                      // total number of layer as other centering
    ...
    if (varName.find(m_depth_average_suffix) == string::npos) {
        ...
        rv->SetNumberOfTuples(ntuples);
        if (varName.find(m_bottom_state_suffix) !=
            string::npos) // for bottom data, same as number of 2D node
        {
            ...
            for (int iNode = 0; iNode < numDataPerLayer; iNode++) {
                ...
                if (layer < valid_bottom_layer) {
                    if (varName.find(m_bottom_state_suffix) != string::npos) {
                        int loc =
                            start_index +
                            1; // the layer just above the first valid layer
                        int max_above_allowd =
                            numTotalLayers - std::max(1, kbp00[iNode]);
...
vtkDataArray* avtSCHISMFileFormatImpl::GetVectorVar(int         a_timeState,
                                                    const char* a_varName) {
    ...
    if (varName.find(m_depth_average_suffix) == string::npos) {
        ...
        rv->SetNumberOfTuples(ntuples);
        if (varName.find(m_bottom_state_suffix) !=
            string::npos) // for surface and bottom data, same as number of 2D
                          // node
        {
            rv->SetNumberOfTuples(numDataPerLayer);
        }
                ...
                if (layer >= bottom_layer) {
                    ...
                    if (varName.find(m_bottom_state_suffix) != string::npos) {
                        int dist =
                            1; // the layer just above the first valid layer
                        int max_above_allowd =
                            num_data_layers - std::max(1, kbp00[iNode]);
                        if (max_above_allowd == 0) // there is only one layer
                        {
                            dist = 0;
                        }
                        for (int iComp = 0; iComp < ncomps; iComp++) {
                            oneEntry[iComp] =
                                valBuff[start_index + dist * ncomps + iComp];
                        }
        ...
        vectorDepthAverage(averageState, valBuff, node_start_index, a_timeState,
                           ncomps);
...
void avtSCHISMFileFormatImpl::prepare_average(int*&      a_kbp,
                                              int*&      a_mapper,
                                              float*&    a_zPtr,
                                              const int& a_timeState) {
...
//
// ****************************************************************************
void avtSCHISMFileFormatImpl::depthAverage(float*     a_averageState,
...
//
// ****************************************************************************
void avtSCHISMFileFormatImpl::vectorDepthAverage(float*     a_averageState,
                                                 float*     a_layeredState,
                                                 int*       a_nodeDataStart,
                                                 const int& a_timeState,
                                                 const int& a_ncomps) {
...
void avtSCHISMFileFormatImpl::getSingleLayerVar(
    float*             a_valBuff,
...
void avtSCHISMFileFormatImpl::Initialize(const std::string& a_data_file) {
    ...
    if (!m_initialized) {
        ...
        if (suffix == "nc") {
        ...
        } else if ((suffix == "61") || (suffix == "62") || (suffix == "63") ||
                   (suffix == "64") || (suffix == "65") || (suffix == "66") ||
                   (suffix == "67") || (suffix == "68") || (suffix == "69") ||
                   (suffix == "70")) {
            ...
            EXCEPTION1(InvalidDBTypeException,
                       "The file is not a valid SCHISM file");
        }
        ...
        std::string zCorFilePath =
            file_dir_str + file_name_str.substr(0, found) + "_zcor.63";
        if (suffix == "nc") {
            // zCorFilePath = m_data_file.substr(0,found)+"_zcor.nc";
            zCorFilePath =
                file_dir_str + file_name_str.substr(0, found) + "_zcor_63.nc";
        }
        ...
        if (!okay) {
            ...
            if (m_data_file_ptr->data_center() == MeshConstants::NODE) {
                m_external_mesh_provider =
                    new ComputeMeshZProvider(m_data_file);
                ...
                if (!ok) {
                    delete m_external_mesh_provider;
                    EXCEPTION1(InvalidDBTypeException,
                               "The file could not be opened for no zcore or "
                               "hgrid mesh exist");
                }
            } else {
                EXCEPTION1(InvalidDBTypeException,
