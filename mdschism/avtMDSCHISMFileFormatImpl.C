...
int avtMDSCHISMFileFormatImpl::GetNTimesteps(const std::string& a_filename) {
    Initialize(a_filename);
    // debug1 << "num time step is"<<m_num_time_step <<"\n";
    return m_num_time_step;
}

// void  avtMDSCHISMFileFormatImpl::ActivateTimestep(const std::string&
// a_filename)
//{
// Initialize(a_filename);
//}
...
avtMDSCHISMFileFormatImpl::~avtMDSCHISMFileFormatImpl() {
    ...
    for (it2 = m_external_mesh_providers.begin();
         it2 != m_external_mesh_providers.end(); it2++) {
        if (it2->second) {
            delete it2->second;
        }
    }
...
void avtMDSCHISMFileFormatImpl::FreeUpResources(void) { return; }

void avtMDSCHISMFileFormatImpl::set_var_name_label_map(
    const std::map<std::string, std::string> a_map) {
    m_var_name_label_map = a_map;
}
void avtMDSCHISMFileFormatImpl::set_var_mesh_map(
    const std::map<std::string, std::string> a_map) {
    m_var_mesh_map = a_map;
}
void avtMDSCHISMFileFormatImpl::set_var_horizontal_center_map(
    const std::map<std::string, std::string> a_map) {
    m_var_horizontal_center_map = a_map;
}
void avtMDSCHISMFileFormatImpl::set_var_vertical_center_map(
    const std::map<std::string, std::string> a_map) {
    m_var_vertical_center_map = a_map;
}
// broadcast string map from rank 0 to all other ranks
void avtMDSCHISMFileFormatImpl::broadCastStringMap(
    std::map<std::string, std::string>& a_map, int myrank) {
#ifdef PARALLEL
    std::vector<std::string> key_list;
    std::vector<std::string> val_list;
    if (myrank == 0) {
        for (std::map<std::string, std::string>::iterator it = a_map.begin();
             it != a_map.end(); ++it) {
...
void avtMDSCHISMFileFormatImpl::BroadcastGlobalInfo(
...
void avtMDSCHISMFileFormatImpl::PopulateDatabaseMetaData(
    avtDatabaseMetaData* a_metaData,
    const std::string    a_data_file,
    int                  a_timeState) {
    ...
    if (PAR_Rank() == 0) {
        ...
        if (m_external_mesh_providers.begin()->second->provide3DMesh()) {
            avtMeshMetaData* mmd = new avtMeshMetaData(
                bounds, extents, mesh_name, nblocks, block_origin, cell_origin,
                group_origin, spatial_dimension, topological_dimension, mt);
            a_metaData->Add(mmd);
        }
        ...
        avtMeshMetaData* mmd1 = new avtMeshMetaData(
            bounds, extents, mesh_name, nblocks, block_origin, cell_origin,
            group_origin, spatial_dimension, topological_dimension, mt);
        a_metaData->Add(mmd1);
        ...
        avtMeshMetaData* mmd2 = new avtMeshMetaData(
            bounds, extents, mesh_name, nblocks, block_origin, cell_origin,
            group_origin, spatial_dimension, topological_dimension, mt);
        a_metaData->Add(mmd2);
        ...
        avtScalarMetaData* smd1 =
            new avtScalarMetaData(m_node_depth_label, mesh, nodeCent);
        a_metaData->Add(smd1);
        ...
        avtScalarMetaData* smd2 = new avtScalarMetaData(
            MeshConstants10::NODE_LEVEL, mesh3d, nodeCent);
        a_metaData->Add(smd2);

        // add 3D element level label
        mesh3d = m_layer_mesh;
        avtScalarMetaData* smd3 =
            new avtScalarMetaData(MeshConstants10::ELE_LEVEL, mesh3d, zoneCent);
        a_metaData->Add(smd3);

        // add 3D side level label
        mesh3d = m_side_center_point_3d_mesh;
        avtScalarMetaData* smd4 = new avtScalarMetaData(
            MeshConstants10::SIDE_LEVEL, mesh3d, nodeCent);
        a_metaData->Add(smd4);

        // add 3D layer lable (for prism center data)
        mesh3d = m_mesh_3d;
        avtScalarMetaData* smd5 =
            new avtScalarMetaData(MeshConstants10::LAYER, mesh3d, zoneCent);
        a_metaData->Add(smd5);
    ...
    // add 3D node level label
    string             mesh3d = m_mesh_3d;
    avtScalarMetaData* smd2 =
        new avtScalarMetaData(MeshConstants10::NODE_LEVEL, mesh3d, nodeCent);
    a_metaData->Add(smd2);
    ...
    // add 3D side level label
    mesh3d = m_side_center_point_3d_mesh;
    avtScalarMetaData* smd4 =
        new avtScalarMetaData(MeshConstants10::SIDE_LEVEL, mesh3d, nodeCent);
    a_metaData->Add(smd4);
...
void avtMDSCHISMFileFormatImpl::addFaceCenterData(
    avtDatabaseMetaData* a_metaData,
...

{
    ...
    if (a_varPtr->num_dims() <= 2) {
        avtScalarMetaData* smd =
            new avtScalarMetaData(a_varLabel, mesh2d, faceCent);
        a_metaData->Add(smd);
    ...
    // vector data
        ...
        if (SCHISMVarIsVector(a_varPtr)) // 3d vector
        {
            ...
            avtVectorMetaData* vmd =
                new avtVectorMetaData(a_varLabel, mesh3d, faceCent, ucomps);
            a_metaData->Add(vmd);
            m_var_mesh_map[a_varLabel] = mesh3d;
            m_var_dim[a_varName] = 3;

            // also add bottom, surface and depth average state option
            avtVectorMetaData* vmd1 =
                new avtVectorMetaData(a_varLabel + m_surface_state_suffix,
                                      m_mesh_2d, faceCent, ucomps);
            a_metaData->Add(vmd1);
            // debug1<<"add  "<<label+m_surface_state_suffix<<" ";
            avtVectorMetaData* vmd2 =
                new avtVectorMetaData(a_varLabel + m_bottom_state_suffix,
                                      m_mesh_2d, faceCent, ucomps);
            a_metaData->Add(vmd2);
            // debug1<<"add  "<<label+m_bottom_state_suffix<<" ";
            avtVectorMetaData* vmd3 =
                new avtVectorMetaData(a_varLabel + m_depth_average_suffix,
                                      m_mesh_2d, faceCent, ucomps);
            a_metaData->Add(vmd3);
        ...
        {
            avtScalarMetaData* smd =
                new avtScalarMetaData(a_varLabel, mesh3d, faceCent);
            a_metaData->Add(smd);
            m_var_mesh_map[a_varLabel] = mesh3d;

            // also add bottom, surface and depth average state option
            a_metaData->Add(new avtScalarMetaData(
                a_varLabel + m_surface_state_suffix, m_mesh_2d, faceCent));
            a_metaData->Add(new avtScalarMetaData(
                a_varLabel + m_bottom_state_suffix, m_mesh_2d, faceCent));
            a_metaData->Add(new avtScalarMetaData(
                a_varLabel + m_depth_average_suffix, m_mesh_2d, faceCent));
            // debug1<<"add  "<< label+m_depth_average_suffix<<" ";
...
void avtMDSCHISMFileFormatImpl::addNodeCenterData(
    avtDatabaseMetaData* a_metaData,
    SCHISMVar10*         a_varPtr,
    const std::string&   a_varName,
    const std::string&   a_varLabel,
    ...
    //  vector var on 2D
        ...
        int          ucomps = (ncomps == 2 ? 3 : ncomps);
        a_metaData->Add(
            new avtVectorMetaData(label, m_mesh_2d, avtCenter, ucomps));
        m_var_mesh_map[label] = m_mesh_2d;
        m_var_dim[varName] = 2;
    }
    //  scalar var having layer dim
        ...
        if (!(varName == MeshConstants10::ZCOORD)) {
            a_metaData->Add(new avtScalarMetaData(
                label + m_surface_state_suffix, m_mesh_2d, avtCenter));
            debug1 << "add  " << label + m_surface_state_suffix << " ";
            a_metaData->Add(new avtScalarMetaData(label + m_bottom_state_suffix,
                                                  m_mesh_2d, avtCenter));
            debug1 << "add  " << label + m_bottom_state_suffix << " ";
            a_metaData->Add(new avtScalarMetaData(
                label + m_depth_average_suffix, m_mesh_2d, avtCenter));
            debug1 << "add  " << label + m_depth_average_suffix << " ";
        ...
        a_metaData->Add(
            new avtVectorMetaData(label, m_mesh_3d, avtCenter, ucomps));
        // AddScalarVarToMetaData(a_metaData,  MeshConstants10::LEVEL,
        // m_mesh_3d, avtCenter);
        m_var_mesh_map[label] = m_mesh_3d;

        // also add bottom, surface and depth average state option
        a_metaData->Add(new avtVectorMetaData(label + m_surface_state_suffix,
                                              m_mesh_2d, avtCenter, ucomps));
        debug1 << "add  " << label + m_surface_state_suffix << " ";
        a_metaData->Add(new avtVectorMetaData(label + m_bottom_state_suffix,
                                              m_mesh_2d, avtCenter, ucomps));
        debug1 << "add  " << label + m_bottom_state_suffix << " ";
        a_metaData->Add(new avtVectorMetaData(label + m_depth_average_suffix,
                                              m_mesh_2d, avtCenter, ucomps));
        debug1 << "add  " << label + m_depth_average_suffix << " ";
        m_var_name_label_map[label + m_surface_state_suffix] = varName;
        m_var_name_label_map[label + m_bottom_state_suffix] = varName;
...
void avtMDSCHISMFileFormatImpl::addSideCenterData(
    avtDatabaseMetaData* a_metaData,
    SCHISMVar10*         a_varPtr,
    const std::string&   a_varName,
    const std::string&   a_varLabel,
    ...
    //  vector var on 2D
        ...
        int          ucomps = (ncomps == 2 ? 3 : ncomps);
        a_metaData->Add(
            new avtVectorMetaData(label, mesh2d, avtCenter, ucomps));
        m_var_mesh_map[label] = mesh2d;
        m_var_dim[varName] = 2;
    }
    //  scalar var having layer dim
        ...
        // also add bottom, surface and depth average state option
        a_metaData->Add(new avtScalarMetaData(label + m_surface_state_suffix,
                                              mesh2d, avtCenter));
        debug1 << "add  " << label + m_surface_state_suffix << " ";
        a_metaData->Add(new avtScalarMetaData(label + m_bottom_state_suffix,
                                              mesh2d, avtCenter));
        debug1 << "add  " << label + m_bottom_state_suffix << " ";
        a_metaData->Add(new avtScalarMetaData(label + m_depth_average_suffix,
                                              mesh2d, avtCenter));
        debug1 << "add  " << label + m_depth_average_suffix << " ";
        ...
        a_metaData->Add(
            new avtVectorMetaData(label, mesh3d, avtCenter, ucomps));
        // AddScalarVarToMetaData(a_metaData,  MeshConstants10::LEVEL, mesh3d,
        // avtCenter);
        m_var_mesh_map[label] = mesh3d;
...
void avtMDSCHISMFileFormatImpl::PopulateStateMetaData(
...
void avtMDSCHISMFileFormatImpl::create2DUnstructuredMesh(
    ...
    if (!m_external_mesh_providers[a_domainID]->fillPointCoord2D(pointPtr,
                                                                 a_timeState)) {
        stringstream msgStream(stringstream::out);
    ...
    long ncell_not_ghost =
    m_external_mesh_providers[a_domainID]->m_number_element_no_ghost; for (long
    i = ncell_not_ghost; i < ncells; i++) gzp[i] = val;*/
    ...
    for (long i = 0; i < npts; i++) {
        long global_id = m_external_mesh_providers[a_domainID]
                             ->m_local_node_id_to_global_id[i];
...
void avtMDSCHISMFileFormatImpl::create2DUnstructuredMeshNoDryWet(
    vtkUnstructuredGrid* a_uGrid,
    long*                a_meshEle,
    const int&           a_domainID,
    const int&           a_timeState) {
...
void avtMDSCHISMFileFormatImpl::createLayerMesh(vtkUnstructuredGrid* a_uGrid,
    ...
    if (!m_external_mesh_providers[a_domainID]->fillPointCoord3D(pointPtr,
                                                                 a_timeState)) {
        stringstream msgStream(stringstream::out);
        msgStream << "Fail to retrieve faces nodes coord at step "
                  << a_timeState;
            ...
            if (iLayer >= (std::max(1, kbe[iCell]) - 1)) {
                ...
                if (numberOfNodeInCell == 3) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        int valid_bottom =
                            std::max(1, m_kbp_node[a_domainID][p]) - 1;
...
void avtMDSCHISMFileFormatImpl::create3DUnstructuredMesh(
    vtkUnstructuredGrid* a_uGrid,
    long*                a_meshEle,
    long*                a_2DPointto3DPoints,
    const int&           a_domainID,
    ...
    for (long i = 0; i < npts; i++) {
        ...
        long global_id = m_external_mesh_providers[a_domainID]
                             ->m_local_node_id_to_global_id[ii];
        int num_domain = m_node_num_domain[global_id - 1];
        if (num_domain > 1)
            gnp[i] = val2;
        else
        ...
        for (int iCell = 0; iCell < m_num_mesh_faces[a_domainID]; ++iCell) {
            {
                ...
                validTopBottomNode(validTopNodeNum, validBottomNodeNum,
                                   ...
                                   nodePtrTemp, m_kbp_node[a_domainID]);

                if (!(prism_bottom_set[iCell])) {
                    if (validBottomNodeNum > 0) {
                        m_kbp_prism[a_domainID][iCell] =
                            iLayer + 1; // level starts from 1 schism tradition
                        prism_bottom_set[iCell] = true;
                    }
                }

                if (numberOfNodeInCell == 3) {
                    insertTriangle3DCell(
                        a_uGrid, validTopNodeNum, validBottomNodeNum,
                        validTopNode, validBottomNode, nodePtrTemp,
                        a_2DPointto3DPoints, m_kbp_node[a_domainID], iCell,
                        iLayer, m_num_layers);
    ...
    m_data_files[a_domainID]->set_prism_bottom(a_timeState,
                                               m_kbp_prism[a_domainID]);
    a_uGrid->GetCellData()->AddArray(ghost_nodes);
...
void avtMDSCHISMFileFormatImpl::create2DPointMesh(vtkUnstructuredGrid* a_uGrid,
    ...
    if (!m_external_mesh_providers[a_domainID]->fillSideCenterCoord2D(
            pointPtr, a_timeState)) {
        stringstream msgStream(stringstream::out);
...
void avtMDSCHISMFileFormatImpl::create3DPointMesh(vtkUnstructuredGrid* a_uGrid,
    ...
    if (!m_external_mesh_providers[a_domainID]->fillSideCenterCoord3D(
            pointPtr, a_timeState)) {
        stringstream msgStream(stringstream::out);
        msgStream << "Fail to retrieve edge center coord at step "
                  << a_timeState;
...
// this is the mesh consists of center at 3d prism side face, used for var like
// flux
void avtMDSCHISMFileFormatImpl::create3DPointFaceMesh(
    vtkUnstructuredGrid* a_uGrid,
    long*                a_meshEle,
    const int&           a_domainID,
    const int&           a_timeState) {
    int numNodes = m_total_valid_3D_side[a_domainID] -
                   (m_external_mesh_providers[a_domainID]->numberOfSide());
    vtkPoints* points = vtkPoints::New();
    points->SetDataTypeToDouble();
    points->GetData()->SetNumberOfComponents(3);
    points->SetNumberOfPoints(numNodes);
    ...
    if (!m_external_mesh_providers[a_domainID]->fillSideFaceCenterCoord3D(
            pointPtr, a_timeState)) {
        stringstream msgStream(stringstream::out);
        msgStream << "Fail to retrieve edge center coord at step "
                  << a_timeState;
...
vtkDataSet* avtMDSCHISMFileFormatImpl::GetMesh(int a_timeState,
                                               int a_domainID,
    ...
    // endl; 	updateMeshZCoordinates(uGrid->GetPoints(), 		a_timeState,
    // a_domainID, 		mesh_name); 	return uGrid;
    // }
    ...
    long* faceNodesPtr =
        new long[m_num_mesh_faces[a_domainID] * (numNodesPerFace + 1)];
    ...
    if (!m_external_mesh_providers[a_domainID]->fillMeshElement(faceNodesPtr)) {
        stringstream msgStream(stringstream::out);
        msgStream << "Fail to retrieve domain faces nodes at step "
                  << a_timeState;
        EXCEPTION3(DBYieldedNoDataException, m_data_file, m_plugin_name,
                   msgStream.str());
    }

    // debug1 << "get face nodes"<<faceNodesPtr[500]<<" "<<faceNodesPtr[501]<<"
    // "<<faceNodesPtr[502]<<"\n";

    if (!m_mesh_is_static) {
        if (m_external_mesh_providers[a_domainID]->update_bottom_layer(
                a_timeState)) {
    ...
    long* m2DPointto3DPoints =
        new long[m_num_mesh_nodes[a_domainID] * m_num_layers];
    ...
    if (!strcmp(mesh_name, m_mesh_2d.c_str())) {
        ...
        create2DUnstructuredMeshNoDryWet(uGrid, faceNodesPtr, a_domainID,
                                         a_timeState);
    } else if (!strcmp(mesh_name, m_layer_mesh.c_str())) {
        createLayerMesh(uGrid, faceNodesPtr, m2DPointto3DPoints, a_domainID,
                        a_timeState);
    } else if (!strcmp(mesh_name, m_mesh_3d.c_str())) {
        create3DUnstructuredMesh(uGrid, faceNodesPtr, m2DPointto3DPoints,
                                 a_domainID, a_timeState);
...
void avtMDSCHISMFileFormatImpl::insertTriangle3DCell(
    vtkUnstructuredGrid* a_uGrid,
    ...
    if (a_validBottomNodeNum == 3) // this is a wedge
    {
        ...
        for (int iNode = 3; iNode < 2 * 3; ++iNode) {
            ...
            verts[iNode] =
                a_2DPointto3DPoints[p * a_num_layers + a_layerID + 1];
        }
...
void avtMDSCHISMFileFormatImpl::insertQuad3DCell(
    vtkUnstructuredGrid* a_uGrid,
    const int&           a_validTopNodeNum,
    const int&           a_validBottomNodeNum,
    long*                a_validTopNode,
...

{
    ...
    for (int inode = 0; inode < 4; inode++) {
        ...
        if ((a_layerID + 2) >= bottomLayer) {
            p_t[inode] =
                a_2DPointto3DPoints[v_t * a_num_layers + a_layerID + 1];
        } else {
            p_t[inode] =
                a_2DPointto3DPoints[v_t * a_num_layers + bottomLayer - 1];
        }
    }
...
void avtMDSCHISMFileFormatImpl::updateMeshZCoordinates(vtkPoints*  a_pointSet,
    ...
    if (it == m_external_mesh_providers.end()) {
        stringstream msgStream(stringstream::out);
        msgStream << "Fail to retrieve domain mesh provider of domain"
                  << a_domainid;
        EXCEPTION1(InvalidVariableException, msgStream.str());
    }
    ...
    if (!strcmp(a_meshName, m_mesh_2d.c_str())) {
        ...
        if (it == m_num_mesh_nodes.end()) {
            stringstream msgStream(stringstream::out);
            msgStream << "Fail to retrieve num of 2d nodes of domain"
                      << a_domainid;
            EXCEPTION1(InvalidVariableException, msgStream.str());
        }
        ...
        if (it == m_total_valid_3D_point.end()) {
            stringstream msgStream(stringstream::out);
            msgStream << "Fail to retrieve num of 3d nodes of domain"
                      << a_domainid;
            EXCEPTION1(InvalidVariableException, msgStream.str());
        }
        ...
        if (it == m_total_valid_3D_side.end()) {
            stringstream msgStream(stringstream::out);
            msgStream << "Fail to retrieve num of 3d side of domain"
                      << a_domainid;
            EXCEPTION1(InvalidVariableException, msgStream.str());
        }
...
void avtMDSCHISMFileFormatImpl::loadAndCacheZSide(const int& a_timeState,
                                                  float*     a_sideCenterZPtr) {
    return;
}

void avtMDSCHISMFileFormatImpl::loadAndCacheZEle(const int& a_timeState,
                                                 float*     a_eleCenterZPtr) {
    return;
}

void avtMDSCHISMFileFormatImpl::loadAndCacheZ(const int& a_timeState,
                                              float*     a_nodeZPtr) {
    return;
}

void avtMDSCHISMFileFormatImpl::load_node_dry_wet(const int& a_time,
                                                  const int& a_domain) {
    ...
    if ((node_dry_wet_cached_time == a_time) && (node_dry_wet)) {
        ...
        // make sure ele dry wet is loaded and cached for this time before fill
        // nodes dry/wet
        ...
        if (!ele_dry_wet) {
            EXCEPTION1(DBYieldedNoDataException,
                       "fill node dry/wet before fill element dry/wet \n");
        }

        m_external_mesh_providers[a_domain]->fill_node_dry_wet(node_dry_wet,
                                                               ele_dry_wet);
...
void avtMDSCHISMFileFormatImpl::load_ele_dry_wet(const int& a_time,
                                                 const int& a_domain) {
    ...
    if (ele_dry_wet_cached_time == a_time) {
        ...
        // debug1 << "done allocate for ele dry/wet for domain " << a_domain <<"
        // "<<num<< "\n";
        ...
        if (!data_file_ptr) {
            EXCEPTION1(InvalidVariableException,
                       ("invalid data file " + m_data_file).c_str());
        }
        ...
        try {
            ...
            EXCEPTION1(InvalidVariableException,
                       ("no " + SCHISMVarName + " in " + m_data_file).c_str());
        }
...
void avtMDSCHISMFileFormatImpl::load_side_dry_wet(const int& a_time,
                                                  const int& a_domain) {
    ...
    if ((side_dry_wet_cached_time == a_time) && (side_dry_wet)) {
        ...
        if (!ele_dry_wet) {
            EXCEPTION1(DBYieldedNoDataException,
                       "fill side dry/wet before fill element dry/wet \n");
        }
        ...
        m_external_mesh_providers[a_domain]->fill_side_dry_wet(side_dry_wet,
                                                               ele_dry_wet);
...
void avtMDSCHISMFileFormatImpl::getMeshDimensions(
    const int& a_domainID, MeshProvider10* a_meshProviderPtr) {
    ...
    m_nominal_size_per_layer[MeshConstants10::NODE +
                             std::to_string(a_domainID)] =
        m_num_mesh_nodes[a_domainID];
    m_nominal_size_per_layer[MeshConstants10::ELEM +
                             std::to_string(a_domainID)] =
        m_num_mesh_faces[a_domainID];
    m_nominal_size_per_layer[MeshConstants10::EDGE +
                             std::to_string(a_domainID)] =
        m_num_mesh_edges[a_domainID];
...
void avtMDSCHISMFileFormatImpl::load_bottom(const int& a_time,
                                            const int& a_domain) {
...
vtkDataArray* avtMDSCHISMFileFormatImpl::getLevel(const string& a_level_name,
                                                  const int&    a_domain) {
    int*        kbp00;
...
vtkDataArray* avtMDSCHISMFileFormatImpl::getBottom(const string& a_bottom_name,
                                                   const int&    a_domain) {
    int*        kbp00;
    std::string data_center(MeshConstants10::NODE);
    long        nominal_num_data_per_Layer = 0;
...
vtkDataArray*
avtMDSCHISMFileFormatImpl::get_node_global_id(const int& a_domain) {
...
vtkDataArray*
avtMDSCHISMFileFormatImpl::get_ele_global_id(const int& a_domain) {
...
vtkDataArray* avtMDSCHISMFileFormatImpl::GetVar(int         a_timeState,
                                                int         a_domainID,
    ...
    SCHISMVar10* SCHISMVarPtr =
        m_data_files[a_domainID]->get_var(SCHISMVarName);
    ...
    if ((!strcmp(a_varName, m_node_depth_label.c_str())) ||
        ...
        getSingleLayerVar(valBuff, m_data_files[a_domainID], a_timeState,
                          SCHISMVarName);
    ...
    debug1 << "loading data for var " << a_varName << " for domain "
           << a_domainID << " " << numDataPerLayer << "\n";
    ...
    if (varName.find(m_depth_average_suffix) == string::npos) {
        ...
        rv->SetNumberOfTuples(ntuples);
        debug1 << " count total num of data on domain " << a_domainID << "  is "
               << ntuples << "\n ";
        ...
        depthAverage(averageState, valBuff, node_start_index, a_timeState,
                     a_domainID, data_center, level_center);
...
vtkDataArray* avtMDSCHISMFileFormatImpl::GetVectorVar(int         a_timeState,
                                                      int         a_domainID,
                                                      const char* a_varName) {
    ...
    if (varName.find(m_depth_average_suffix) == string::npos) {
        ...
        vectorDepthAverage(averageState, valBuff, node_start_index, a_timeState,
                           a_domainID, ncomps, data_center, level_center);
...
void avtMDSCHISMFileFormatImpl::prepare_average(
    int*&              a_kbp,
    int*&              a_mapper,
    float*&            a_zPtr,
    const int&         a_timeState,
    ...
    if (a_horizontal_center == MeshConstants10::EDGE) {
        ...
        if (!m_external_mesh_providers[a_domainID]->zSideCenter3D(
                a_zPtr, a_timeState)) {
            stringstream msgStream(stringstream::out);
            msgStream << "Fail to retrieve side z at step " << a_timeState;
            EXCEPTION3(DBYieldedNoDataException, m_data_file, m_plugin_name,
        ...
        if (!m_external_mesh_providers[a_domainID]->zEleCenter3D(a_zPtr,
                                                                 a_timeState)) {
            stringstream msgStream(stringstream::out);
        ...
        debug1 << "load z for average size "
               << m_total_valid_3D_point[a_domainID] << "\n";
        // loadAndCacheZ(a_timeState,a_zPtr);
        if (!m_external_mesh_providers[a_domainID]->zcoords3D(a_zPtr,
                                                              a_timeState)) {
            stringstream msgStream(stringstream::out);
            msgStream << "Fail to retrieve node z at step " << a_timeState;
            EXCEPTION3(DBYieldedNoDataException, m_data_file, m_plugin_name,
...
//
// ****************************************************************************
void avtMDSCHISMFileFormatImpl::depthAverage(
    ...
    prepare_average(kbp, mapper, zPtr, a_timeState, a_domain,
                    a_horizontal_center, a_vertical_center);
...
//
// ****************************************************************************
void avtMDSCHISMFileFormatImpl::vectorDepthAverage(
    float*             a_averageState,
    float*             a_layeredState,
    long*              a_nodeDataStart,
    const int&         a_timeState,
...
void avtMDSCHISMFileFormatImpl::getSingleLayerVar(
    float*             a_valBuff,
...
void avtMDSCHISMFileFormatImpl::count_node_side_num_domain(
    const std::string& a_path, const int& num_proc) {
        ...
        for (int i = 0; i < num_proc; i++) {
            ...
            if (!localFileStream->good()) {
                EXCEPTION1(InvalidFilesException,
                           local_global_file + " is not valid");
            }
            std::string lineTemp;
...
int avtMDSCHISMFileFormatImpl::load_per_proc_file(const std::string& a_path,
                                                  int&               num_node,
                                                  int&               num_side,
                                                  int& num_ele) const {
...
int avtMDSCHISMFileFormatImpl::num_domain() { return m_number_domain; }
// ****************************************************************************
...
void avtMDSCHISMFileFormatImpl::Initialize(std::string a_data_file) {
    ...
    if (!m_initialized) {
        ...
        // debug1 << m_data_file_path + "\\" + "schout_" + "0000" + "_" +
        // data_file_part2+"\n";
        int nproc = 1;
        int myrank = 0;
#ifdef PARALLEL
        ...
        int ndomain = 0;
        if (myrank == 0) {
            ndomain = load_per_proc_file(m_data_file_path, m_global_num_node,
                                         m_global_num_side, m_global_num_ele);
        }
        debug1 << "global num before broadcast:" << m_global_num_node << " "
               << m_global_num_ele << " " << m_global_num_side << "\n";
#ifdef PARALLEL
        BroadcastInt(ndomain);
        BroadcastInt(m_global_num_node);
        BroadcastInt(m_global_num_ele);
        BroadcastInt(m_global_num_side);
        debug1 << "global num after broadcast:" << m_global_num_node << " "
               << m_global_num_ele << " " << m_global_num_side << "\n";
#endif
        ...
        //BroadcastIntArray(buff2, 20000); 	debug1 << "finished broadcast domain
        //num arr\n";
        // }
        // if (myrank != 0)
        //{
    ...
    std:
        ...
        int num_domain_resides =
            ndomain_per_thread + (myrank < oneExtraUntil ? 1 : 0);
        ...
        debug1 << "mesh has been distributed num domain resides:"
               << num_domain_resides << "\n";
        ...
        for (int i = 0; i < num_domain_resides; i++) {
            ...
            std::string domain_str = ss.str();
            // std::string domain_data_file = data_file_part1 + domain_str + "_"
            // + data_file_part2;
            std::string domain_data_file = m_data_file_path + pt + "schout_" +
                                           domain_str + data_file_part2;
            debug1 << "loading " << domain_data_file << "\n";
            std::string local_mesh_file =
                m_data_file_path + pt + "local_to_global_" + domain_str;
            MDSchismOutput* a_data_file_ptr = NULL;
            try {
                debug1 << "creating file ptr " << i << "\n";
                a_data_file_ptr =
                    new MDSchismOutput(domain_data_file, local_mesh_file);
                debug1 << "created file ptr " << a_data_file_ptr << "\n";
            } catch (SCHISMFileException10 e) {
                EXCEPTION1(InvalidDBTypeException, e.what());
            } catch (...) {
                EXCEPTION1(InvalidDBTypeException,
                           "not valid schsim NC output");
            }
            m_data_files[id] = a_data_file_ptr;

            MDSCHISMMeshProvider* a_local_mesh_ptr = NULL;
            try {
                a_local_mesh_ptr = new MDSCHISMMeshProvider(
                    domain_data_file, a_data_file_ptr, local_mesh_file);
        ...
        // m_dry_wet_flag =
        // m_data_files.begin()->second->get_dry_wet_val_flag();
        ...
        for (it = m_external_mesh_providers.begin();
             it != m_external_mesh_providers.end(); it++) {
        ...
        m_mesh_is_static =
            m_external_mesh_providers.begin()->second->mesh3d_is_static();
...
void avtMDSCHISMFileFormatImpl::getTime() {
    ...
    if (m_num_time_step) {
        ...
        SCHISMVar10* SCHISMTimePtr =
            m_data_files.begin()->second->get_var(m_time);
