...
ZCoordMeshProvider::ZCoordMeshProvider(const std::string& a_zcoordFile)
    : SCHISMMeshProvider(a_zcoordFile) {}

bool ZCoordMeshProvider::zcoords3D(float*     a_zCachePtr,
    ...
    if (!(zVarPtr->is_valid())) {
        ...
        throw SCHISMFileException("invlaid var " + MeshConstants::ZCOORD +
    ...
    if (!(zVarPtr->get(zPtr))) {
        throw SCHISMFileException("fail to retrieve var " +
                                  MeshConstants::ZCOORD + " from data file " +
                                  m_dataFilePtr->file());
    }

    // zcore z stored as layer dimension change first,here convert to node dim
    // change first
    ...
    for (int iLayer = 0; iLayer < m_number_layer; iLayer++) {
        ...
        for (int iNode = 0; iNode < m_number_node; iNode++) {
            ...
            if (iLayer < (std::max(1, m_kbp00[iNode]) - 1)) {
                ...
                *a_zCachePtr = zPtr[start_index + iLayer + 1 -
                                    std::max(1, m_kbp00[iNode])];
                a_zCachePtr++;
            }
        }
    }
...
bool ZCoordMeshProvider::zcoords3D2(float*     a_zCachePtr,
                                    const int& a_timeStep) const {
    int timeStart = a_timeStep;
