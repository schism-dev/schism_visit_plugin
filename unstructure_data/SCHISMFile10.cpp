...
SCHISMAtt10::SCHISMAtt10(const std::string& a_attName)
    : m_name(a_attName),
      m_num_str_value(0),
      m_num_float_value(0),
      m_num_int_value(0),
...
std::string SCHISMAtt10::name() const { return m_name; }

void SCHISMAtt10::add_string_value(const std::string& a_value) {
    if (m_num_str_value >= MAX_ATT_NUM_10) {
        std::cerr << "attribute capacity limit is reached, new attribute value "
                     "is not added";
    } else {
...
bool SCHISMAtt10::is_valid() const { return m_is_valid; }
...
SCHISMDim10::SCHISMDim10() : m_is_valid(false), m_name("junkdim"), m_size(0) {}

SCHISMDim10::~SCHISMDim10() {}

bool SCHISMDim10::is_valid() const { return m_is_valid; }

int SCHISMDim10::size() const { return m_size; }

int SCHISMDim10::id() const { return m_id; }

std::string SCHISMDim10::name() const { return m_name; }

SCHISMVar10::SCHISMVar10(const std::string& a_varName)
    : m_is_valid(true),
      m_data_cached(false),
      m_name(a_varName),
...
int SCHISMVar10::num_dims() const { return m_num_dim; }
...
bool SCHISMVar10::add_att(SCHISMAtt10* a_att) {
    if (m_num_attributes >= MAX_ATT_NUM_10) {
        std::cerr << "variable " << m_name
                  << ":attributes capacity is reached, can't add more\n";
        return false;
    } else {
...
bool SCHISMVar10::add_dimension(const int& a_dimID) {
    if (m_num_dim >= MAX_DIM_NUM_10) {
        std::cerr << "variable " << m_name
                  << ":dimension capacity is reached, can't add more\n";
        return false;
    } else {
...
std::string SCHISMVar10::name() const { return m_name; }
...
bool SCHISMVar10::is_valid() const { return m_is_valid; }

bool SCHISMVar10::put_a_float(const float& a_value, int* a_dim1Loc) {
    return false;
}

bool SCHISMVar10::get(int* a_buffer) { return false; }

bool SCHISMVar10::get(float* a_buffer) { return false; }

bool SCHISMVar10::get(double* a_buffer) { return false; }

bool SCHISMVar10::get(int* a_buffer, int* a_bottom) { return false; }

bool SCHISMVar10::get(float* a_buffer, int* a_bottom) { return false; }

bool SCHISMVar10::get(double* a_buffer, int* a_bottom) { return false; }

void SCHISMVar10::set_cur(const int& a_timeRecord) { return; }

bool SCHISMVar10::get(long* a_buffer, int* a_bottom) { return false; }

bool SCHISMVar10::get(long* a_buffer) { return false; }

bool SCHISMVar10::get_int_cache(int* a_buffer) { return false; }

bool SCHISMVar10::get_long_cache(long* a_buffer) { return false; }

bool SCHISMVar10::get_float_cache(float* a_buffer) { return false; }
...
int SCHISMVar10::num_component() const { return m_num_component; }
...
SCHISMFile10::SCHISMFile10(const std::string a_SCHISMOutputFile)
    : m_total_num_vars(0),
      m_total_num_dims(0),
...
int SCHISMFile10::get_dry_wet_val_flag() { return 1; }
...
std::string SCHISMFile10::level_center() const { return m_layer_center; }

std::string SCHISMFile10::data_center() const { return m_data_center; }

void SCHISMFile10::set_mesh_data_ptr(SCHISMFile10* a_ptr) { return; }
...
bool SCHISMFile10::none_data_var(const std::string a_varName) const {
    if ((a_varName == m_mesh_face_nodes) || (a_varName == m_node_x) ||
        (a_varName == m_node_y) || (a_varName == m_node_depth) ||
        (a_varName == m_node_surface) || (a_varName == m_layer_scoord)) {
        return true;
    } else {
        return false;
    }
}
std::string SCHISMFile10::file() const { return m_SCHISMOutputFile; }
bool        SCHISMFile10::is_valid() const { return m_is_valid; }

int SCHISMFile10::num_vars() const { return m_total_num_vars; }

int SCHISMFile10::num_dims() const { return m_total_num_dims; }
...
SCHISMDim10* SCHISMFile10::get_dim(const int& a_dimID) const {
    if (a_dimID < m_total_num_dims) {
        ...
        throw SCHISMFileException10(
            "input dimension id is larger than total  number of dim\n");
    }
}
...
SCHISMDim10* SCHISMFile10::get_invalidDim() const { return new SCHISMDim10(); }
...
SCHISMVar10* SCHISMFile10::get_var(const int& a_varID) const {
    if (a_varID < m_total_num_vars) {
        ...
        throw SCHISMFileException10(
            "input var id is larger than total  number of var\n");
    }
}
...
// empty api
bool SCHISMFile10::read(char* a_buffer, const int& a_numByte) { return false; }
...
// empty api
void SCHISMFile10::set_cur(const int& a_TIMEStep, const int& a_extraOffset) {
    return;
}
...
bool SCHISMFile10::update_bottom_index(const int& a_time) { return false; }
...
std::string
SCHISMFile10::global_att_as_string(const std::string& a_att_name) const {
    throw SCHISMFileException10("global att is not implemented\n");
    return "";
}
