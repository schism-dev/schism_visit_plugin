...
NetcdfSchismOutput::NetcdfSchismOutput(const std::string a_outputFile)
    : SCHISMFile(a_outputFile) {
...
bool NetcdfSchismOutput::load_dim_var() {
    ...
    for (int ivar = 0; ivar < num_var; ivar++) {
        ...
        if (m_vector_component_map.find(var_name) !=
            m_vector_component_map.end()) {
        ...
        if (var_name_added.find(var_name) != var_name_added.end()) {
            ...
            static_cast<NetcdfSchismOutputVar*>(m_variables[loc])
                ->fill_ncVar(a_var);
            continue;
        }
    ...
    for (int ivar = 0; ivar < m_total_num_vars; ivar++) {
        ...
        if (num_com > 1) {
            ...
            SCHISMDim* wrapped_dim =
                newSCHISMDim(name + "_component", idim, num_com);
            m_dimensions[idim] = wrapped_dim;
...
bool NetcdfSchismOutput::cache_face_nodes(SCHISMVar* mesh_var) {
    ...
    NcDim* dimFacePtr =
        m_outputNcFilePtr->get_dim(MeshConstants::DIM_MESH_FACES.c_str());

    if (dimFacePtr->is_valid()) {
        numMeshFaces = dimFacePtr->size();
    } else {
        throw SCHISMFileException("no element dim in data file " +
                                  m_SCHISMOutputFile);
    }

    int* faceNodesPtr =
        new int[numMeshFaces * MeshConstants::MAX_NUM_NODE_PER_CELL];
    NcVar* ncFacePtr =
        m_outputNcFilePtr->get_var(MeshConstants::MESH_FACE_NODES.c_str());
    if (!(ncFacePtr->is_valid())) {

        throw SCHISMFileException("No face node in data file " +
                                  m_SCHISMOutputFile);
    }

    if (!ncFacePtr->get(faceNodesPtr, numMeshFaces,
                        MeshConstants::MAX_NUM_NODE_PER_CELL)) {

        throw SCHISMFileException("fail to load face node in data file " +
                                  m_SCHISMOutputFile);
        ;
    }

    int* nodes =
        new int[numMeshFaces * (MeshConstants::MAX_NUM_NODE_PER_CELL + 1)];
...
bool NetcdfSchismOutputVar::put_a_float(const float& a_value, int* a_dim1Loc) {
    return false;
}

void NetcdfSchismOutputVar::fill_ncVar(NcVar* a_nc_var) {
    if (m_num_component == MAX_VAR_COMPONENT) {
        throw SCHISMFileException(m_name +
                                  " having component more than allowd");
    }
...
bool NetcdfSchismOutputVar::get(int* a_buffer) const {
    ...
    for (int idim = 0; idim < num_dim; idim++) {
        ...
        if ((dim_name == MeshConstants::DIM_TIME) &&
            (m_name != MeshConstants::TIME)) {
...
bool NetcdfSchismOutputVar::get(float* a_buffer) const {
    ...
    int num_bottom_duplicate =
        0; // for half level data, bottom data are extra data need to get rid of
...
NetcdfSchismOutputVar::NetcdfSchismOutputVar(const std::string& a_varName)
    : SCHISMVar(a_varName) {
    m_num_component = 0;
}
