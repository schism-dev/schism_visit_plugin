...
class SCHISMDim10 {
  ...
  private:
    SCHISMDim10(const std::string& a_name, const int& a_id, const int& a_size);
...
class SCHISMVar10 {
  public:
    ...
    virtual bool put_a_float(const float& a_value, int* a_dim1Loc);
...
class SCHISMFile10 {
  public:
    ...
    virtual void set_cur(const int& a_TIMEStep, const int& a_extraOffset);
    std::string  file() const;
    SCHISMDim10*
    newSCHISMDim(const std::string& a_name, const int& a_id, const int& a_size);
    std::string level_center() const;
    std::string data_center() const;

    virtual bool update_bottom_index(
        const int& a_time); // this function will update mesh node/edge/element
                            // bottom according to zcor filled value, it should
                            // be called when mesh bottom changes. if bottom
                            // changes it will return true, or false. for mesh
                            // with fixed bottom it is not used.

    virtual int
    get_dry_wet_val_flag(); // 0: filled with last wetting val 1: junk
    ...
    virtual std::string
    global_att_as_string(const std::string& a_att_name) const;
