...
void retrieve1DVar(float*             a_valBuff,
    ...
    if (!(SCHISMPtr->is_valid())) {
        ...
        throw SCHISMFileException("invlaid var " + a_varName +
                                  " for data file " + a_SCHISMFilePtr->file());
    } else {

        if (!SCHISMPtr->get(a_valBuff)) {
            throw SCHISMFileException("fail to retrieve var " + a_varName +
                                      " from data file " +
                                      a_SCHISMFilePtr->file());
        }
    }
}
...
void retrieve2DVar(float*             a_valBuff,
    ...
    if (!(SCHISMVarPtr->is_valid())) {
        ...
        throw SCHISMFileException("invlaid var " + a_varName +
                                  " for data file " + a_SCHISMOutPtr->file());
    }
    ...
    if (!SCHISMVarPtr->get(a_valBuff)) {
        throw SCHISMFileException("fail to retrieve var " + a_varName +
                                  " from data file " + a_SCHISMOutPtr->file());
    }
}

// function extract file name,dir, and ext from a path,source:internet
void        decomposePath(const char* filePath,
                          char*       fileDir,
                          char*       fileName,
                          char*       fileExt) {
#if defined _WIN32
    ...
    if (fileName)
        _snprintf(fileName, MAX_PATH, "%.*s", startOfExt - startOfName,
                  startOfName);
    ...
    if (fileName)
        snprintf(fileName, MAX_PATH, "%.*s", startOfExt - startOfName,
                 startOfName);
