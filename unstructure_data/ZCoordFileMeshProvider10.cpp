...
ZCoordMeshProvider10::ZCoordMeshProvider10(const std::string& a_meshFile)
    : SCHISMMeshProvider10(a_meshFile),
      m_mesh_file(a_meshFile) {

    size_t found = m_mesh_file.find_last_of("/\\");
std:
    string data_file_path = m_mesh_file.substr(0, found);
    bool   scribeIO = false;
    // find out format of output by reading global attribute source
    // if "V10" in the string 5.8 format, otherwise scribeIo format (no atribute
    // or other values)
    try {
        std::string source =
            m_dataFilePtr->global_att_as_string(MeshConstants10::source);
...
bool ZCoordMeshProvider10::zcoords3D(float*     a_zCachePtr,
    ...
    if (m_dataFilePtr->inquire_var(MeshConstants10::ZCOORD)) {
        ...
        // first try to load from zcor_*.nc, for output might be in the new
        // scriber format
        ...
        if (m_zcor_file_ptr) {
            if (m_zcor_file_ptr->is_valid()) {
                ...
                return false;
                // throw SCHISMFileException10("No valid zcor data for file " +
                // m_dataFilePtr->file());
            }
        } else {
            return false;
    ...
    retrieve1DVar(nodeDepthPtr, m_dataFilePtr, MeshConstants10::NODE_DEPTH,
                  m_number_node);
    ...
    if (outZCoor) {
        if (!(zVarPtr->get(zPtr, kbp00))) {
            throw SCHISMFileException10(
                "fail to retrieve var " + MeshConstants10::ZCOORD2 +
                " from data file " + m_zcor_file_ptr->file());
        }
    } else {
        if (!(zVarPtr->get(zPtr))) {
            throw SCHISMFileException10(
                "fail to retrieve var " + MeshConstants10::ZCOORD +
...
bool ZCoordMeshProvider10::zcoords3D2(float*     a_zCachePtr,
                                      const int& a_timeStep) const {
    int timeStart = a_timeStep;
    ...
    retrieve1DVar(node_depth_ptr, m_dataFilePtr, MeshConstants10::NODE_DEPTH,
                  m_number_node);
            ...
            if (iLayer < (std::max(1, kbp00[iNode]) - 1)) {
                ...
                if ((temp == dry_zcor) || (temp == missing_val)) {
                    ...
                    if (m_layerSCoords) {
                        ...
                        float z = convertStoZ(sigma, surface, depth, m_hs, m_hc,
                                              m_thetab, m_thetaf);
                        zPtr[start_index + iLayer + 1 -
                             std::max(1, kbp00[iNode])] = z;
                    } else {
                        float depth = node_depth_ptr[iNode];
                        float z = surface - depth * iLayer / m_number_layer;
                        zPtr[start_index + iLayer + 1 -
