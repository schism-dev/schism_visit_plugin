...
class MDSchismOutput : public SCHISMFile10 {
  public:
    MDSchismOutput(const std::string a_mdoutputFile,
...
class MDSchismOutputVar : public SCHISMVar10 {
  public:
    ...
    bool put_a_float(const float& a_value, int* a_dim1Loc);
  ...
  protected:
    ...
    template <class T> bool load_from_file(T* a_buffer);
...
template <class T> bool MDSchismOutputVar::load_from_file(T* a_buffer) {
    ...
    for (int idim = 0; idim < num_dim; idim++) {
        ...
        if ((dim_name == MeshConstants10::DIM_TIME) &&
            (m_name != MeshConstants10::TIME)) {
    ...
    // debug1 << " done load dims in load from file "<<buffer_size<<"
    // "<<node_num<<" "<<num_layer<<"\n";
    ...
    last_node_valid_record_len =
        (num_layer - max(1, bottom_layer[node_num - 1]) + 1) * num_component;
    ...
    for (long inode = 0; inode < node_num; inode++) {
        ...
        if (inode < (node_num - 1)) {
            ...
            a_node_valid_data_end =
                a_node_valid_data_start + last_node_valid_record_len - 1;
        }

        long a_node_valid_record_len =
            a_node_valid_data_end - a_node_valid_data_start + 1;

        long valid_record_start_in_nc_buffer =
            (inode + 1) * a_node_record_length_in_nc_buffer -
            a_node_valid_record_len;
        long nc_buffer_id = valid_record_start_in_nc_buffer;
        for (long valid_data_index = a_node_valid_data_start;
             valid_data_index < (a_node_valid_data_end + 1);
             valid_data_index++) {
            a_buffer[valid_data_index] = buffer[nc_buffer_id];
            nc_buffer_id++;
        }
        // debug1 << "done read valid ncvar data "<<inode<<" " <<
        // valid_record_start_in_nc_buffer<<"\n";
