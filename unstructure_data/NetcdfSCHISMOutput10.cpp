...
NetcdfSchismOutput10::NetcdfSchismOutput10(const std::string a_outputFile)
    : SCHISMFile10(a_outputFile),
      m_face_bottom(NULL),
      m_node_bottom(NULL),
      m_edge_bottom(NULL),
            ...
            if (att_id == source_id) {
                ...
                if ((source.find("SCHISM model output")) == std::string::npos) {
                    throw SCHISMFileException10(
                        a_outputFile +
                        "is not a valid SCHISM NC output file\n");
                }
            }
        }
    }
    ...
    // if (!(find_source_att))
    //{
    //	throw SCHISMFileException10( a_outputFile+"is not a valid SCHSIM NC
    // output file\n");
    // }
    ...
    try {
        ...
        throw SCHISMFileException10(a_outputFile +
                                    "has long int attribute which is not "
                                    "supported by current netcdf c++ lib\n");
    }
...
int NetcdfSchismOutput10::get_dry_wet_val_flag() // 0: filled with last wetting
                                                 // val 1: junk
{
    ...
    try {
        NcVar* ncvar =
            m_outputNcFilePtr->get_var(MeshConstants10::DRY_VALUE_FLAG.c_str());
        int val;
        int count[1];
        count[0] = 1;
...
int NetcdfSchismOutput10::global_att_as_int(
    const std::string& a_att_name) const {
...
std::string NetcdfSchismOutput10::global_att_as_string(
    const std::string& a_att_name) const {
    int num_att = m_outputNcFilePtr->num_atts();
    for (int i = 0; i < num_att; i++) {
        NcAtt*      a_att = m_outputNcFilePtr->get_att(i);
    ...
    if (has_var(node_bottom_name)) {
        ...
        if (ncvar->is_valid()) {
            ...
            try {
                ...
                throw SCHISMFileException10(
                    this->file() + "：node dim is invalid or not exist\n");
            }
            ...
            try {
                ...
                throw SCHISMFileException10(
                    this->file() + "：layer dim is invalid or not exist\n");
            }
            long nLayers = 0;
...
void NetcdfSchismOutput10::fill_edge_bottom() {
    if (has_var(MeshConstants10::EDGE_BOTTOM)) {
        ...
        if (ncvar->is_valid()) {
            ...
            try {
                ...
                throw SCHISMFileException10(
                    this->file() + "：face dim is invalid or not exist\n");
            }
            long numMeshFaces = 0;
            ...
            try {
                ...
                throw SCHISMFileException10(
                    this->file() + "：edge dim is invalid or not exist\n");
            }
            long numMeshEdges = 0;
    ...
    } else if (m_node_bottom) // infer from node bottom if node bottom filled
    {
        if (has_var(MeshConstants10::MESH_FACE_NODES)) {
            ...
            numMeshNodes = dimNodePtr->size();
            long numSide =
                meshSideNum(faceNodesPtr, numMeshFaces, numMeshNodes);

            long* sideNodes = new long[numSide * 2];
            meshSideNode(sideNodes, faceNodesPtr, numSide, numMeshFaces,
                         numMeshNodes);
...
void NetcdfSchismOutput10::fill_ele_bottom() {
    if (has_var(MeshConstants10::FACE_BOTTOM)) {
        NcVar* ncvar =
            m_outputNcFilePtr->get_var(MeshConstants10::FACE_BOTTOM.c_str());
        if (ncvar->is_valid()) {
            NcDim* dimFacePtr = m_outputNcFilePtr->get_dim(
                MeshConstants10::DIM_MESH_FACES.c_str());
            long numMeshFaces = 0;
    ...
    } else if (m_node_bottom) // infer from node bottom if node bottom filled
    {
        if (has_var(MeshConstants10::MESH_FACE_NODES)) {
            ...
            for (long iface = 0; iface < numMeshFaces; iface++) {
                int num_point_in_face =
                    faceNodesPtr[iface *
                                 (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1)];
                int min_node_bottom = MeshConstants10::BIG_LEVEL;
                for (int inode = 0; inode < num_point_in_face; inode++) {
                    long node_id = faceNodesPtr
                        [iface * (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1) +
                         inode + 1]; // node id is 1 based
...
void NetcdfSchismOutput10::get_node_bottom(int*       a_node_bottom,
                                           const int& a_time) {
    if ((m_node_bottom) && (a_node_bottom)) {
        NcDim* dimNodePtr =
            m_outputNcFilePtr->get_dim(MeshConstants10::DIM_MESH_NODES.c_str());
        long numMeshNodes = 0;
        numMeshNodes = dimNodePtr->size();
...
void NetcdfSchismOutput10::get_face_bottom(int*       a_face_bottom,
                                           const int& a_time) {
    if ((m_node_bottom) && (a_face_bottom)) {
        NcDim* dimFacePtr =
            m_outputNcFilePtr->get_dim(MeshConstants10::DIM_MESH_FACES.c_str());
        long numMeshFaces = 0;
        numMeshFaces = dimFacePtr->size();
...
void NetcdfSchismOutput10::get_edge_bottom(int*       a_edge_bottom,
                                           const int& a_time) {
    if ((m_node_bottom) && (a_edge_bottom)) {
        NcDim* dimEdgePtr =
            m_outputNcFilePtr->get_dim(MeshConstants10::DIM_MESH_EDGES.c_str());
        long numMeshEdges = 0;
        numMeshEdges = dimEdgePtr->size();
...
void NetcdfSchismOutput10::set_mesh_bottom(SCHISMFile10* a_ptr,
                                           const int&    a_time) {
    if (!m_node_bottom) {
        NcDim* dimNodePtr =
            m_outputNcFilePtr->get_dim(MeshConstants10::DIM_MESH_NODES.c_str());
...
bool NetcdfSchismOutput10::update_node_bottom(const int& a_time) {
    if (m_node_bottom) {
        ...
        NcVar* ncvar =
            m_outputNcFilePtr->get_var(MeshConstants10::ZCOORD.c_str());
...
void NetcdfSchismOutput10::update_edge_bottom(const int& a_time,
                                              int*       a_node_bottom) {
...
void NetcdfSchismOutput10::update_ele_bottom(const int& a_time,
                                             int*       a_node_bottom) {
    ...
    long*        nodes =
        new long[numMeshFaces * (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1)];
    ...
    for (long iface = 0; iface < numMeshFaces; iface++) {
        int num_point_in_face =
            nodes[iface * (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1)];
        int min_node_bottom = MeshConstants10::BIG_LEVEL;
        for (int inode = 0; inode < num_point_in_face; inode++) {
            long node_id =
                nodes[iface * (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1) +
...
bool NetcdfSchismOutput10::load_dim_var() {
    ...
    for (int idim = 0; idim < num_dim; idim++) {
        ...
        if (a_dim->name() == MeshConstants10::DIM_MESH_FACE_NODES) {
            wrapped_dim = newSCHISMDim(a_dim->name(), idim, a_dim->size() + 1);
        } else {
            wrapped_dim = newSCHISMDim(a_dim->name(), idim, a_dim->size());
        }
    ...
    for (int ivar = 0; ivar < num_var; ivar++) {
        ...
        NetcdfSchismOutputVar10* schism_var =
            new NetcdfSchismOutputVar10(var_name);
        ...
        for (int i_att_var = 0; i_att_var < num_att_var; i_att_var++) {
            ...
            if (type == ncChar) {
                ...
                throw std::invalid_argument(
                    "current version of nectd c++ api doesn't support long int "
                    "attribute!");
            ...
            // if
            // ((!(nc_att_name.compare(MeshConstants10::CENTER)))||(!(nc_att_name.compare(MeshConstants10::LOCATION))))
            //{
...
bool NetcdfSchismOutput10::cache_face_nodes(SCHISMVar10* mesh_var) {
    ...
    if (dimFacePtr->is_valid()) {
        ...
        throw SCHISMFileException10("no element dim in data file " +
                                    m_SCHISMOutputFile);
    }

    long* faceNodesPtr =
        new long[numMeshFaces * MeshConstants10::MAX_NUM_NODE_PER_CELL];
    NcVar* ncFacePtr =
        m_outputNcFilePtr->get_var(MeshConstants10::MESH_FACE_NODES.c_str());
    if (!(ncFacePtr->is_valid())) {

        throw SCHISMFileException10("No face node in data file " +
                                    m_SCHISMOutputFile);
    }

    if (!ncFacePtr->get(faceNodesPtr, numMeshFaces,
                        MeshConstants10::MAX_NUM_NODE_PER_CELL)) {

        throw SCHISMFileException10("fail to load face node in data file " +
                                    m_SCHISMOutputFile);
        ;
    }
...
void NetcdfSchismOutputVar10::fill_ncVar(NcVar* a_nc_var) {
    ...
    for (int iatt = 0; iatt < a_nc_var->num_atts(); iatt++) {
        ...
        if (!att_name.compare(
                "ivs")) // this is the label for number of vector components
        {
...
NetcdfSchismOutputVar10::NetcdfSchismOutputVar10(const std::string& a_varName)
    : SCHISMVar10(a_varName) {
    m_num_component = 0;
}
