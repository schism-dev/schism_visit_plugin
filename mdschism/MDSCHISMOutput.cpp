...
MDSchismOutput::MDSchismOutput(const std::string a_outputFile,
                               const std::string a_local_mesh_file)
    ...
    //      {
    //	      throw SCHISMFileException10( a_outputFile+"is not a valid SCHSIM
    // NC output file\n");
    //      }
    //  }
    //}
...
std::string
MDSchismOutput::global_att_as_string(const std::string& a_att_name) const {
    int num_att = m_outputNcFilePtr->num_atts();
    for (int i = 0; i < num_att; i++) {
        NcAtt*      a_att = m_outputNcFilePtr->get_att(i);
        std::string att_name = a_att->name();
...
void MDSchismOutput::fill_bottom() {
    ...
    (*localFileStream) >> nrec >> v2 >> nspool >> number_layer >> kz >> h0 >>
        hs >> hc >> thetab >> thetaf >> ics;
...
void MDSchismOutput::update_ele_bottom(const int& a_time, int* a_node_bottom) {
    ...
    for (long iface = 0; iface < numMeshFaces; iface++) {
        int num_point_in_face =
            m_face_nodes[iface * (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1)];
...
void MDSchismOutput::update_prism_bottom(const int& a_time,
                                         int*       a_prism_bottom) {
    ...
    for (int ivar = 0; ivar < num_var; ivar++) {
        ...
        for (int i_att_var = 0; i_att_var < num_att_var; i_att_var++) {
            ...
            if (!(nc_att_name.compare(MeshConstants10::I23D))) {
                ...
                if (!(i23d % 3)) {
                    schism_var->set_vertical_center(
                        MeshConstants10::HALF_LAYER);
                } else {
                    schism_var->set_vertical_center(
                        MeshConstants10::FULL_LAYER);
                }
            }
        }
...
bool MDSchismOutputVar::put_a_float(const float& a_value, int* a_dim1Loc) {
    return false;
}
...
MDSchismOutputVar::MDSchismOutputVar(const std::string& a_varName)
    : SCHISMVar10(a_varName) {
    m_num_component = 0;
}
MDSchismOutputVar::MDSchismOutputVar() : SCHISMVar10() { m_num_component = 0; }
