...
NativeSchismOutputVar::NativeSchismOutputVar(const std::string& a_varName)
    : SCHISMVar(a_varName) {}
...
bool NativeSchismOutputVar::put_a_float(const float& a_value, int* a_dimLoc) {
    ...
    for (int iDim = 0; iDim < dimSize; iDim++) {
        ...
        if (iDim < (dimSize - 1)) {
            SCHISMDim* nextDim =
                m_schismfilePtr->get_dim(m_dimensions[iDim + 1]);
...
bool NativeSchismOutputVar::get(float* a_buffer) const {
    ...
    if (m_data_cached) {
        ...
        if (!hasLayer) {
            ...
            m_schismfilePtr->read(floatArrayBuffer.cBuffer,
                                  dataSize * DATA_NUM_BYTE);
            ...
            int staggeredDataSize =
                computeDataNumPerTIMEStepWitHStaggeredLayers();
            floatArrayBuffer.cBuffer =
                new char[staggeredDataSize * DATA_NUM_BYTE];
            m_schismfilePtr->read(floatArrayBuffer.cBuffer,
                                  staggeredDataSize * DATA_NUM_BYTE);
            // nodeDim is dim 1
            ...
            for (int iNode = 0; iNode < totalNodeNum; iNode++) {
                ...
                for (int iLayer = valid_bottom_layer; iLayer < numLayer + 1;
                     iLayer++) {
                    for (int iComponent = 0; iComponent < comSize;
                         iComponent++) {
                        // int numDataLayer=numLayer-max(1,m_kbp00[iNode])+1;
                        // int outDataIndex = iNode*numDataLayer*comSize +
                        // (iLayer -1) * comSize + iComponent;
                        //  remove extra bottom data for half level dataset
                        if ((level_center != MeshConstants::HALF_LAYER) ||
                            ((level_center == MeshConstants::HALF_LAYER) &&
                             (iLayer > valid_bottom_layer))) {
                            a_buffer[out_buffer_index] =
                                floatArrayBuffer.floatArray[read_buffer_index];
                            out_buffer_index++;
                        }
                        read_buffer_index++;
                    }
...
NativeSchismOutput::NativeSchismOutput(const std::string a_SCHISMOutputFile)
    : SCHISMFile(a_SCHISMOutputFile),
      m_numByte(4),
      m_dataBlockBeginOffset(0),
      m_dataBlockLength(0) {
    m_schismOutputFileStream =
        new ifstream(a_SCHISMOutputFile.c_str(), ios::binary);
...
void NativeSchismOutput::set_cur(const int& a_step, const int& a_extraOffset) {
...
bool NativeSchismOutput::load_dims_Vars() {
    ...
    m_schismOutputFileStream->read(readHeaderBuffer.cBuffer,
                                   12 * DATA_NUM_BYTE);
    ...
    m_schismOutputFileStream->read(floatArrayBuffer.cBuffer,
                                   (kz - 1) * DATA_NUM_BYTE);
    m_schismOutputFileStream->read(floatArrayBuffer.cBuffer,
                                   (nvrt - kz + 1) * DATA_NUM_BYTE);

    int        nSigmaDim = m_total_num_dims;
    SCHISMDim* sigmaDim =
        newSCHISMDim(MeshConstants::DIM_SIGMA_LAYERS, nSigmaDim, nvrt - kz + 1);
    m_dimensions[nSigmaDim] = sigmaDim;
    m_total_num_dims++;

    int        nvDim = m_total_num_dims;
    SCHISMDim* vDim = newSCHISMDim(MeshConstants::DIM_LAYERS, nvDim, nvrt);
    m_dimensions[nvDim] = vDim;
    m_total_num_dims++;

    int        nzDim = m_total_num_dims;
    SCHISMDim* zDim = newSCHISMDim(MeshConstants::DIM_KZ_LAYERS, nzDim, kz - 1);
    ...
    if (ivs > 1) {
        ...
        SCHISMDim* comDim =
            newSCHISMDim(MeshConstants::DIM_VAR_COMPONENT, nComponentDim, ivs);
        m_dimensions[nComponentDim] = comDim;
        m_total_num_dims++;
    }

    NativeSchismOutputVar* sigmaVar =
        new NativeSchismOutputVar(MeshConstants::LAYER_SCOORD);
    ...
    // it might be number of node(61-64), ele (70) or sides (67) depends on file
    // type
    ...
    try {
        ...
        cout << "file name " << m_SCHISMOutputFile
             << " don't contain SCHISM output type surfix (61-70)\n";
        return false;
    }
    ...
    if ((fileTypeInt == 67) || (fileTypeInt == 68) ||
        (fileTypeInt ==
         65)) // nodes of mesh defined by original mesh side center
    {
    ...
    SCHISMDim* nodeDim =
        newSCHISMDim(node_dim_name, nNodeDim, totalNumberOfNodes);
    m_dimensions[nNodeDim] = nodeDim;
    m_total_num_dims++;

    NativeSchismOutputVar* bottom_var =
        new NativeSchismOutputVar(node_bottom_name);
    ...
    for (int iCell = 0; iCell < totalNumberOfEle; iCell++) {
        ...
        if ((num_node_in_cell < 3) || (num_node_in_cell > 4)) {
            std::string invalidCellNum("invalid cell number %i, must be 3 or 4",
                                       num_node_in_cell);
            throw SCHISMFileException(invalidCellNum);
        } else if (num_node_in_cell == 3) {
            m_schismOutputFileStream->read(threeIntArrayBuffer.cBuffer,
                                           3 * DATA_NUM_BYTE);
            ...
            m_schismOutputFileStream->read(fourIntArrayBuffer.cBuffer,
                                           4 * DATA_NUM_BYTE);
            nodes[index] = num_node_in_cell;
    ...
    SCHISMDim* cellDim =
        newSCHISMDim(m_dim_mesh_faces, nCellDim, totalNumberOfEle);
    m_dimensions[nCellDim] = cellDim;
    m_total_num_dims++;

    int        nCellNodeDim = m_total_num_dims;
    SCHISMDim* cellNodeDim =
        newSCHISMDim(MeshConstants::DIM_MESH_FACE_NODES, nCellNodeDim,
                     (MAX_CELL_NODE + 1)); // a extra position for cell number
    m_dimensions[nCellNodeDim] = cellNodeDim;
    m_total_num_dims++;

    NativeSchismOutputVar* cellNodesVar =
        new NativeSchismOutputVar(m_mesh_face_nodes);
    ...
    SCHISMDim* TIMEDim = newSCHISMDim(m_dim_time, nTIMEDim, nrec);
    ...
    NativeSchismOutputVar* surfaceVar =
        new NativeSchismOutputVar(m_node_surface);
    ...
    if (i23d == 2) {
        ...
        for (int iNode = 0; iNode < totalNumberOfNodes; iNode++) {
            singleRecordByteSize +=
                ivs * DATA_NUM_BYTE * (nvrt - max(1, kbp00[iNode]) + 1);
        }
    }
    ...
    for (int iRec = 0; iRec < nrec; iRec++) {
        ...
        m_schismOutputFileStream->read(singleFloatBuffer.fourChar,
                                       DATA_NUM_BYTE);
        TIMEValues[iRec] = *(singleFloatBuffer.floatValue);
        m_schismOutputFileStream->seekg(singleRecordByteSize - DATA_NUM_BYTE,
                                        ios::cur);
    }
    ...
    if (!(startPos == std::string::npos)) {
        // SCHISMOutput  =
        // m_SCHISMOutputFile.substr(startPos+1,endPos-startPos-1);
        SCHISMOutput = fileNameStr.substr(startPos + 1);
    } else {
        cout << "file name " << m_SCHISMOutputFile
             << " don't contain valid data name\n";
    }
