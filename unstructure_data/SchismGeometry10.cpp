...
int local_3side_node[3][2] = {{1, 2}, {2, 0}, {0, 1}};

int local_4side_node[4][2] = {{1, 2}, {2, 3}, {3, 0}, {0, 1}};
...
long meshSideNum(long*       a_meshNode,
    ...
    for (long iele = 0; iele < a_numberEle; iele++) {
        ...
        for (long iNode = 0; iNode < numNode; iNode++) {
            long node =
                a_meshNode[iele * (max_node_in_cell + 1) + 1 + iNode] - 1;
            numEleAtNode[node] += 1;
        }
    }
    ...
    for (long iele = 0; iele < a_numberEle; iele++) {
        ...
        for (int iSide = 0; iSide < numSide; iSide++) {
            ...
            for (int iNeiborEle = 0; iNeiborEle < numEleAtNode[node1];
                 iNeiborEle++) {
                ...
                if ((eleIndex > invalid_id) && (eleIndex != iele)) {
                    ...
                    for (int iiNode = 0; iiNode < numNode; iiNode++) {
                        long nodeIndex =
                            a_meshNode[eleStartIndex + 1 + iiNode] - 1;
                        if (nodeIndex == node2) {
                            shareThisSide = true;
                            break;
                        }
                    ...
                    if (shareThisSide) {
                        neiborEleAllSide[max_node_in_cell * iele + iSide] =
                            eleIndex;
                    }
                }
            }
...
void meshSideNodeNeiborEle(long*       a_mesh_node,
    ...
    for (long iele = 0; iele < a_number_ele; iele++) {
        ...
        for (long iNode = 0; iNode < numNode; iNode++) {
            long node =
                a_mesh_node[iele * (max_node_in_cell + 1) + 1 + iNode] - 1;
    ...
    a_side_neighbor_ele =
        new long[2 * a_number_side]; // only max 2 element neibor a side
    ...
    for (long iele = 0; iele < a_number_ele; iele++) {
        ...
        for (int iSide = 0; iSide < numSide; iSide++) {
            ...
            for (int iNeiborEle = 0; iNeiborEle < num_ele_at_node[node1];
                 iNeiborEle++) {
                long eleIndex =
                    a_node_neighbor_ele[node1 * maxEleAtNode + iNeiborEle];
                if ((eleIndex > invalid_id) && (eleIndex != iele)) {
                    long eleStartIndex = eleIndex * (max_node_in_cell + 1);
                    ...
                    for (int iiNode = 0; iiNode < numNode; iiNode++) {
                        long nodeIndex =
                            a_mesh_node[eleStartIndex + 1 + iiNode] - 1;
                        if (nodeIndex == node2) {
                            shareThisSide = true;
                            break;
                        }
                    ...
                    if (shareThisSide) {
                        ele_side_neighbor_ele[max_node_in_cell * iele + iSide] =
                            eleIndex;
                    }
                }
            }
