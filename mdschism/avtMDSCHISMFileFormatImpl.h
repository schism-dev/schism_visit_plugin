...
// has to put mpi.h before any netcdf.h include to avoid redefining of
// mpi_comm,... such as.
#ifdef PARALLEL
#include <mpi.h>
#endif
...
class avtMDSCHISMFileFormatImpl : public FileFormatFavorInterface {
  public:
    ...
    vtkDataArray*
    GetVar(int a_timeState, int a_domainID, const char* a_varName);

    vtkDataArray*
    GetVectorVar(int a_timeState, int a_domainID, const char* a_varName);
    ...
    void set_var_horizontal_center_map(
        const std::map<std::string, std::string> a_map);
    void
    set_var_vertical_center_map(const std::map<std::string, std::string> a_map);
  ...
  protected:
    ...
    void getMeshDimensions(const int&      a_dominID,
    ...
    bool
    fourPointsCoplanar(double p1[3], double p2[3], double p3[3], double p4[3]);
    ...
    void          broadCastStringMap(std::map<std::string, std::string>& m_map,
                                     int                                 myrank);
    int           load_per_proc_file(const std::string& a_path,
                                     int&               num_node,
                                     int&               num_side,
                                     int&               num_ele) const;
    void          count_node_side_num_domain(const std::string& a_path,
                                             const int&         num_proc);
  ...
  private:
    ...
    int m_dry_wet_flag; // 0: dry cell/ele/side filled with last wetting val, 1:
                        // junk
    ...
    // store num of domain a side/node resides to figure out inter-facial
    // node/side and mark them as ghost
