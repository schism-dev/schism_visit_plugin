...
ComputeMeshZProvider::ComputeMeshZProvider(const std::string& a_selfeOutputFile)
    : SCHISMMeshProvider(a_selfeOutputFile) {}
...
bool ComputeMeshZProvider::zcoords3D(float*     a_zCachePtr,
                                     const int& a_timeStep) const {
    float* surfacePtr;
    surfacePtr = new float[m_number_node];

    retrieve2DVar(surfacePtr, m_dataFilePtr, a_timeStep,
                  MeshConstants::NODE_SURFACE);

    float* nodeDepthPtr = new float[m_number_node];

    retrieve1DVar(nodeDepthPtr, m_dataFilePtr, MeshConstants::NODE_DEPTH,
                  m_number_node);
...
bool ComputeMeshZProvider::zcoords3D2(float*     a_zCachePtr,
                                      const int& a_timeStep) const {
    float* surfacePtr;
    surfacePtr = new float[m_number_node];
