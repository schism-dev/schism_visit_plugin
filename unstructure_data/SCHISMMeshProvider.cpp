...
SCHISMMeshProvider::SCHISMMeshProvider(const std::string& a_fileHasMeshData)
    : MeshProvider(a_fileHasMeshData),
      m_kbp00(NULL),
      m_layerSCoords(NULL),
      m_kbs(NULL),
      m_kbe(NULL),
...
void SCHISMMeshProvider::genElementSide() {
    int* meshElementNodes =
        new int[(MeshConstants::MAX_NUM_NODE_PER_CELL + 1) * m_number_element];

    fillMeshElement(meshElementNodes);

    m_number_side =
        meshSideNum(meshElementNodes, m_number_element, m_number_node);

    m_sideNodes = new int[m_number_side * 2];
    meshSideNode(m_sideNodes, meshElementNodes, m_number_side, m_number_element,
                 m_number_node);
    ...
    for (int iEle = 0; iEle < m_number_element; iEle++) {
        ...
        for (int iNode = 1; iNode < numNode; iNode++) {
            int node =
                meshElementNodes[iEle * (maxNodeInCell + 1) + 1 + iNode] - 1;
...
void SCHISMMeshProvider::loadMeshDim() {
    ...
    SCHISMDim* dimNodePtr =
        m_dataFilePtr->get_dim(MeshConstants::DIM_MESH_NODES);
    ...
    retrieve1DVar(m_kbp00, m_dataFilePtr, MeshConstants::NODE_BOTTOM,
                  m_number_node);

    // SELFEDim constructor and destructor are private, no need to del.
    SCHISMDim* dimFacePtr =
        m_dataFilePtr->get_dim(MeshConstants::DIM_MESH_FACES);
    ...
    if (!(sVarPtr->is_valid())) {
        ...
        if (!sVarPtr->get(m_layerSCoords)) {
            throw SCHISMFileException(
                "fail to retrieve var " + MeshConstants::LAYER_SCOORD +
...
// z is filled with 0
bool SCHISMMeshProvider::fillPointCoord2D(float*     a_pointCoord,
    ...
    if (m_mesh_loaded == false) {
        ...
        retrieve1DVar(nodeXPtr, m_dataFilePtr, MeshConstants::NODE_X,
                      m_number_node);

        retrieve1DVar(nodeYPtr, m_dataFilePtr, MeshConstants::NODE_Y,
                      m_number_node);
...
bool SCHISMMeshProvider::fillSideCenterCoord3D(float*     a_pointCoord,
                                               const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
            ...
            for (int iSide = 0; iSide < m_number_side; iSide++) {
                ...
                if (iLayer >= (std::max(1, m_kbs[iSide]) - 1)) {
                    ...
                    *a_pointCoord++ = half * (y1 + y2);
                    ...
                    int z_displace_node1 =
                        iLayer - std::max(1, m_kbp00[node1]) + 1;
                    int z_displace_node2 =
                        iLayer - std::max(1, m_kbp00[node2]) + 1;
...
bool SCHISMMeshProvider::fillSideFaceCenterCoord3D(
...
// z is filled with 0
bool SCHISMMeshProvider::fillSideCenterCoord2D(float*     a_pointCoord,
                                               const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
bool SCHISMMeshProvider::fillEleCenterCoord3D(float*     a_pointCoord,
                                              const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
        ...
        int* meshElementNodes =
            new int[(MeshConstants::MAX_NUM_NODE_PER_CELL + 1) *
                    m_number_element];
        fillMeshElement(meshElementNodes);
            ...
            for (int iEle = 0; iEle < m_number_element; iEle++) {
                ...
                if (iLayer >= (std::max(1, m_kbe[iEle]) - 1)) {
                    ...
                    for (int iNode = 0; iNode < numNode; iNode++) {
                        int node = meshElementNodes[iEle * (maxNodeInCell + 1) +
                                                    1 + iNode] -
                                   1;
...
// z is filled with 0
bool SCHISMMeshProvider::fillEleCenterCoord2D(float*     a_pointCoord,
                                              const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
        ...
        for (int iEle = 0; iEle < m_number_element; iEle++) {
            ...
            for (int iNode = 0; iNode < numNode; iNode++) {
                int node =
                    meshElementNodes[iEle * (maxNodeInCell + 1) + 1 + iNode] -
                    1;
                x_sum += nodeXPtr[iNode];
                y_sum += nodeYPtr[iNode];
            }
...
bool SCHISMMeshProvider::fillPointCoord3D(float*     a_pointCoord,
                                          const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
bool SCHISMMeshProvider::fillMeshElement(int* a_elementCache) const {
    if (!m_mesh_loaded) {
        ...
        SCHISMVar* ncFacePtr =
            m_dataFilePtr->get_var(MeshConstants::MESH_FACE_NODES);
        if (!(ncFacePtr->is_valid())) {

            throw SCHISMFileException("no mesh element in " +
                                      m_dataFilePtr->file());
        }

        if (!ncFacePtr->get(a_elementCache)) {
            throw SCHISMFileException("fail to read mesh element in " +
                                      m_dataFilePtr->file());
        }
    }
    return true;
...
bool SCHISMMeshProvider::zcoords2D(float*     a_zCachePtr,
                                   const int& a_timeStep) const {
    for (int iNode = 0; iNode < m_number_node; iNode++) {
...
bool SCHISMMeshProvider::zcoords3D(float*     a_zCachePtr,
                                   const int& a_timeStep) const {

    return false;
}

bool SCHISMMeshProvider::zcoords3D2(float*     a_zCachePtr,
                                    const int& a_timeStep) const {

    return false;
}

bool SCHISMMeshProvider::zSideCenter2D(float*     a_zCachePtr,
                                       const int& a_timeStep) const {
...
bool SCHISMMeshProvider::zSideCenter3D(float*     a_zCachePtr,
                                       const int& a_timeStep) const {
    float* zPtr = new float[m_number_layer * m_number_node];
    float* zPtrTemp = zPtr;
...
bool SCHISMMeshProvider::zEleCenter2D(float*     a_zCachePtr,
                                      const int& a_timeStep) const {
    for (int i = 0; i < m_number_element; i++) {
...
bool SCHISMMeshProvider::zEleCenter3D(float*     a_zCachePtr,
                                      const int& a_timeStep) const {
    float* zPtr = new float[m_number_layer * m_number_node];
    float* zPtrTemp = zPtr;
    bool   temp = zcoords3D2(zPtr, a_timeStep);
    ...
    for (int iLayer = 0; iLayer < m_number_layer; iLayer++) {
        ...
        for (int iEle = 0; iEle < m_number_element; iEle++) {
            ...
            if (iLayer >= (std::max(1, m_kbe[iEle]) - 1)) {
                ...
                for (int iNode = 0; iNode < numNode; iNode++) {
                    int node = meshElementNodes[iEle * (maxNodeInCell + 1) + 1 +
                                                iNode] -
                               1;
                    int kbp = m_kbp00[node];
...
bool SCHISMMeshProvider::depth(float* a_cache) const { return false; }
