...
int test_netcdf4(std::string & a_ncfile)
{
	...
	{
		cout<<"Error:"<<a_ncfile<<" is not opend correclty"<<endl;
		return 0;
	}
	else
	{
		cout<<a_ncfile<<" is opend correclty"<<endl;
		return 1;
	}
}

// those commnted lines are not compatible with current
// code NC C++ API
// this function testing open nc file using NC C++ 4.3 api
// which chanes greately from 4.1. This function should also
...
int testZCoreProvider(std::string& file1,std::string& out)
{
	...
	for(int i=0;i<numNode;i++)
	{
		if (i>0) start_id[i]=start_id[i-1]+zvar_size[i];
