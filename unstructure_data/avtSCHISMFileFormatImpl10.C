...
avtSCHISMFileFormatImpl10::avtSCHISMFileFormatImpl10()
    ...
    Vector3DVarMapX["horizontalSideVelX"] =
        "horizontalSideVelX;horizontalSideVelY";
    Vector3DVarMapY["horizontalSideVelY"] =
        "horizontalSideVelX;horizontalSideVelY";
    Vector3DVarMapX["waveForceX"] = "waveForceX;waveForceY";
    Vector3DVarMapY["waveForceY"] = "waveForceX;waveForceY";
    Vector3DVarMapX["horzontalViscosityX"] =
        "horzontalViscosityX;horzontalViscosityY";
    Vector3DVarMapY["horzontalViscosityY"] =
        "horzontalViscosityX;horzontalViscosityY";
    ...
    Vector3DVarMapX["verticalViscosityX"] =
        "verticalViscosityX;verticalViscosityY";
    Vector3DVarMapY["verticalViscosityY"] =
        "verticalViscosityX;verticalViscosityY";
    Vector3DVarMapX["mommentumAdvectionX"] =
        "mommentumAdvectionX;mommentumAdvectionY";
    Vector3DVarMapY["mommentumAdvectionY"] =
        "mommentumAdvectionX;mommentumAdvectionY";

    Vector2DVarMapX["airPressureGradientX"] =
        "airPressureGradientX;airPressureGradientY";
    Vector2DVarMapY["airPressureGradientY"] =
        "airPressureGradientX;airPressureGradientY";
    Vector2DVarMapX["tidePotentialGradX"] =
        "tidePotentialGradX;tidePotentialGradY";
    Vector2DVarMapY["tidePotentialGradY"] =
        "tidePotentialGradX;tidePotentialGradY";
    ...
    Vector2DVarMapX["sedBedloadTransportX"] =
        "sedBedloadTransportX;sedBedloadTransportY";
    Vector2DVarMapX["sedBedloadTransportY"] =
        "sedBedloadTransportX;sedBedloadTransportY";
...
int avtSCHISMFileFormatImpl10::GetNTimesteps(const std::string& a_filename) {
...
void avtSCHISMFileFormatImpl10::FreeUpResources(void) {
    if (m_time_ptr) {
        delete m_time_ptr;
    }
...
void avtSCHISMFileFormatImpl10::PopulateDatabaseMetaData(
    ...
    if (m_external_mesh_provider->provide3DMesh()) {
        a_avtFile->addMeshToMetaData(a_metaData, mesh_name, mt, extents,
                                     nblocks, block_origin, spatial_dimension,
    ...
    a_avtFile->addMeshToMetaData(a_metaData, mesh_name, mt, extents, nblocks,
    ...
    if (!m_scribeIO) {
        a_avtFile->addScalarVarToMetaData(a_metaData, m_node_depth_label, mesh,
                                          nodeCent);
    ...
    // MeshConstants10::NODE_SURFACE_LABEL, mesh, nodeCent);
    ...
    a_avtFile->addScalarVarToMetaData(a_metaData, MeshConstants10::NODE_LEVEL,
                                      mesh3d, nodeCent);

    // add 3D element level label
    mesh3d = m_layer_mesh;
    a_avtFile->addScalarVarToMetaData(a_metaData, MeshConstants10::ELE_LEVEL,
                                      mesh3d, zoneCent);

    // add 3D side level label
    mesh3d = m_side_center_point_3d_mesh;
    a_avtFile->addScalarVarToMetaData(a_metaData, MeshConstants10::SIDE_LEVEL,
                                      mesh3d, nodeCent);

    // add 3D layer lable (for prism center data)
    mesh3d = m_mesh_3d;
    a_avtFile->addScalarVarToMetaData(a_metaData, MeshConstants10::LAYER,
                                      mesh3d, zoneCent);

    // add node bottom label
    mesh = m_mesh_2d;
    // a_avtFile->addScalarVarToMetaData(a_metaData,
    // MeshConstants10::NODE_BOTTOM, mesh, nodeCent);
    // a_avtFile->addScalarVarToMetaData(a_metaData,
    // MeshConstants10::FACE_BOTTOM, mesh, zoneCent);

    // a_avtFile->addScalarVarToMetaData(a_metaData,
    // MeshConstants10::EDGE_BOTTOM, mesh, nodeCent);

    PopulateStateMetaData(a_metaData, a_avtFile, a_timeState);
    debug1 << "finish populate metadata \n";
}

void avtSCHISMFileFormatImpl10::addFaceCenterData(
    avtDatabaseMetaData* a_metaData,
...

{
    ...
    if (a_varPtr->num_dims() <= 2) {
        a_avtFile->addScalarVarToMetaData(a_metaData, a_varLabel, mesh2d,
                                          faceCent);
    ...
    // vector data
        ...
        if (!(m_external_mesh_provider->provide3DMesh())) {
            ...
            msgStream << "3D variable " << a_varLabel
                      << " is not supported by a 2D meshprovider\n";
        ...
        if (SCHISMVarIsVector(a_varPtr)) // 3d vector
        {
            ...
            a_avtFile->addVectorVarToMetaData(a_metaData, a_varLabel, mesh3d,
                                              faceCent, ucomps);
            m_var_mesh_map[a_varLabel] = mesh3d;
            m_var_dim[a_varName] = 3;

            // also add bottom, surface and depth average state option
            a_avtFile->addVectorVarToMetaData(
                a_metaData, a_varLabel + m_surface_state_suffix, m_mesh_2d,
                faceCent, ucomps);
            // debug1<<"add  "<<label+m_surface_state_suffix<<" ";
            a_avtFile->addVectorVarToMetaData(
                a_metaData, a_varLabel + m_bottom_state_suffix, m_mesh_2d,
                faceCent, ucomps);
            // debug1<<"add  "<<label+m_bottom_state_suffix<<" ";
            a_avtFile->addVectorVarToMetaData(
                a_metaData, a_varLabel + m_depth_average_suffix, m_mesh_2d,
                faceCent, ucomps);
            // debug1<<"add  "<< label+m_depth_average_suffix<<" ";
            m_var_name_label_map[a_varLabel + m_surface_state_suffix] =
                a_varName;
            m_var_name_label_map[a_varLabel + m_bottom_state_suffix] =
                a_varName;
            m_var_name_label_map[a_varLabel + m_depth_average_suffix] =
                a_varName;
        ...
        {
            a_avtFile->addScalarVarToMetaData(a_metaData, a_varLabel, mesh3d,
                                              faceCent);
            m_var_mesh_map[a_varLabel] = mesh3d;

            // also add bottom, surface and depth average state option
            a_avtFile->addScalarVarToMetaData(
                a_metaData, a_varLabel + m_surface_state_suffix, m_mesh_2d,
                faceCent);
            // debug1<<"add  "<<label+m_surface_state_suffix<<" ";
            a_avtFile->addScalarVarToMetaData(
            ...
            a_avtFile->addScalarVarToMetaData(
                ...
                faceCent);
            // debug1<<"add  "<< label+m_depth_average_suffix<<" ";
            //  all those surface, botootm and average are based on original
            //  data set
            m_var_name_label_map[a_varLabel + m_surface_state_suffix] =
                a_varName;
            m_var_name_label_map[a_varLabel + m_bottom_state_suffix] =
        ...
        // AddScalarVarToMetaData(a_metaData,  MeshConstants10::LEVEL, mesh3d,
        // faceCent);
    } else {
    }
}
void avtSCHISMFileFormatImpl10::addNodeCenterData(
    avtDatabaseMetaData* a_metaData,
    SCHISMVar10*         a_varPtr,
    avtSCHISMFileFormat* a_avtFile,
    const std::string&   a_varName,
    ...
    //  scalar var on 2D
    if (a_varPtr->num_dims() <= 2) {
        a_avtFile->addScalarVarToMetaData(a_metaData, label, m_mesh_2d,
                                          avtCenter);
    ...
    //  vector var on 2D
        ...
        int          ncomps = comDim->size();
        int          ucomps = (ncomps == 2 ? 3 : ncomps);
        a_avtFile->addVectorVarToMetaData(a_metaData, label, m_mesh_2d,
                                          avtCenter, ucomps);
        m_var_mesh_map[label] = m_mesh_2d;
        m_var_dim[varName] = 2;
    }
    //  scalar var having layer dim
        ...
        // AddScalarVarToMetaData(a_metaData,  MeshConstants10::LEVEL,
        // m_mesh_3d, avtCenter);
        a_avtFile->addScalarVarToMetaData(a_metaData, label, m_mesh_3d,
                                          avtCenter);
        ...
        if (!(varName == MeshConstants10::ZCOORD)) {
            ...
            // all those surface, botootm and average are based on original data
            // set
        ...
        a_avtFile->addVectorVarToMetaData(a_metaData, label, m_mesh_3d,
                                          avtCenter, ucomps);
        // AddScalarVarToMetaData(a_metaData,  MeshConstants10::LEVEL,
        // m_mesh_3d, avtCenter);
        m_var_mesh_map[label] = m_mesh_3d;
...
void avtSCHISMFileFormatImpl10::addSideCenterData(
    avtDatabaseMetaData* a_metaData,
    SCHISMVar10*         a_varPtr,
    avtSCHISMFileFormat* a_avtFile,
    const std::string&   a_varName,
    ...
    //  vector var on 2D
        ...
        int          ncomps = comDim->size();
        int          ucomps = (ncomps == 2 ? 3 : ncomps);
        a_avtFile->addVectorVarToMetaData(a_metaData, label, mesh2d, avtCenter,
                                          ucomps);
        m_var_mesh_map[label] = mesh2d;
        m_var_dim[varName] = 2;
    }
    //  scalar var having layer dim
        ...
        // also add bottom, surface and depth average state option
        a_avtFile->addScalarVarToMetaData(
            a_metaData, label + m_surface_state_suffix, mesh2d, avtCenter);
        debug1 << "add  " << label + m_surface_state_suffix << " ";
        a_avtFile->addScalarVarToMetaData(
            a_metaData, label + m_bottom_state_suffix, mesh2d, avtCenter);
        debug1 << "add  " << label + m_bottom_state_suffix << " ";
        a_avtFile->addScalarVarToMetaData(
            a_metaData, label + m_depth_average_suffix, mesh2d, avtCenter);
        debug1 << "add  " << label + m_depth_average_suffix << " ";
        ...
        a_avtFile->addVectorVarToMetaData(a_metaData, label, mesh3d, avtCenter,
                                          ucomps);
        // AddScalarVarToMetaData(a_metaData,  MeshConstants10::LEVEL, mesh3d,
        // avtCenter);
        m_var_mesh_map[label] = mesh3d;
        ...
        a_avtFile->addVectorVarToMetaData(a_metaData,
                                          label + m_surface_state_suffix,
                                          mesh2d, avtCenter, ucomps);
        debug1 << "add  " << label + m_surface_state_suffix << " ";
        a_avtFile->addVectorVarToMetaData(a_metaData,
                                          label + m_bottom_state_suffix, mesh2d,
                                          avtCenter, ucomps);
        debug1 << "add  " << label + m_bottom_state_suffix << " ";
        a_avtFile->addVectorVarToMetaData(a_metaData,
                                          label + m_depth_average_suffix,
...
void avtSCHISMFileFormatImpl10::PopulateStateMetaData(
    avtDatabaseMetaData* a_metaData,
    avtSCHISMFileFormat* a_avtFile,
    int                  a_timeState) {
    ...
    for (int iVar = 0; iVar < numVar; iVar++) {
        ...
        if ((itX != Vector3DVarMapX.end()) || (itY != Vector3DVarMapY.end())) {
            ...
            {
                a_avtFile->addVectorVarToMetaData(a_metaData, label, mesh3d,
                                                  avtCenter, ucomps);
                // a_avtFile->addScalarVarToMetaData(a_metaData,varName,
                // m_mesh_3d, avtCenter);
                ...
                // also add bottom, surface and depth average state option
                a_avtFile->addVectorVarToMetaData(
                    a_metaData, label + m_surface_state_suffix, mesh2d,
                    avtCenter, ucomps);
                // debug1 << "add  " << label + m_surface_state_suffix << " ";
                a_avtFile->addVectorVarToMetaData(a_metaData,
                                                  label + m_bottom_state_suffix,
                ...
                // debug1 << "add  " << label + m_bottom_state_suffix << " ";
                a_avtFile->addVectorVarToMetaData(
                    a_metaData, label + m_depth_average_suffix, mesh2d,
                    avtCenter, ucomps);
                // debug1 << "add  " << label + m_depth_average_suffix << " ";
        ...
        if ((itX != Vector2DVarMapX.end()) || (itY != Vector2DVarMapY.end())) {
            ...
            if ((varName[varName.length() - 1] == 'Y') ||
                (varName[varName.length() - 1] == 'X')) {
                label = varName.substr(0, varName.length() - 1);
            } else {
                std::size_t found = varName.find('_');
                if (found != std::string::npos) {
                    label = varName.substr(0, found - 1) +
                            varName.substr(found, varName.length());
                } else {
            ...
            if (std::find(out2d_vector_added.begin(), out2d_vector_added.end(),
                          label) == out2d_vector_added.end()) {
                a_avtFile->addVectorVarToMetaData(a_metaData, label, mesh2d,
                                                  avtCenter, ucomps);
                m_var_name_label_map[label] = vars;
                m_var_mesh_map[label] = mesh2d;
        ...
        if (location == FACE) {
            addFaceCenterData(a_metaData, varPtr, a_avtFile, varName, label,
                              avtCenter);
        } else if (location == NODE) {
            addNodeCenterData(a_metaData, varPtr, a_avtFile, varName, label,
                              avtCenter);
        } else if (location == SIDE) {
            addSideCenterData(a_metaData, varPtr, a_avtFile, varName, label,
                              avtCenter);
        }
...
void avtSCHISMFileFormatImpl10::create2DUnstructuredMesh(
    vtkUnstructuredGrid* a_uGrid, long* a_meshEle, const int& a_timeState) {
    ...
    if (!m_external_mesh_provider->fillPointCoord2D(pointPtr, a_timeState)) {
        stringstream msgStream(stringstream::out);
        msgStream << "Fail to retrieve faces nodes coord at step "
                  << a_timeState;
        EXCEPTION3(DBYieldedNoDataException, m_data_file, m_plugin_name,
                   msgStream.str());
    }
...
void avtSCHISMFileFormatImpl10::create2DUnstructuredMeshNoDryWet(
    vtkUnstructuredGrid* a_uGrid, long* a_meshEle, const int& a_timeState) {
    long       numNodes = m_num_mesh_nodes;
    vtkPoints* points = vtkPoints::New();
    points->SetNumberOfPoints(numNodes);
        ...
        for (int iCell = 0; iCell < m_num_mesh_faces; ++iCell) {
            ...
            if (iLayer >= (std::max(1, kbe[iCell]) - 1)) {
                ...
                if (numberOfNodeInCell == 3) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        int p =
                            a_meshEle[iCell * (NODESPERELE + 1) + i + 1] - 1;
                        int p3d =
                            a_2DPointto3DPoints[p * m_num_layers + iLayer];
                        int valid_bottom = std::max(1, m_kbp_node[p]) - 1;
                        if (iLayer < valid_bottom) {
                            p3d = a_2DPointto3DPoints[p * m_num_layers +
                                                      valid_bottom];
                        }
...
void avtSCHISMFileFormatImpl10::create3DUnstructuredMesh(
    vtkUnstructuredGrid* a_uGrid,
    long*                a_meshEle,
    long*                a_2DPointto3DPoints,
    const int&           a_timeState) {
    vtkPoints* points = vtkPoints::New();
        ...
        for (int iCell = 0; iCell < m_num_mesh_faces; ++iCell) {
            {
                nodePtrTemp =
                    a_meshEle +
                    (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1) * iCell;
                int numberOfNodeInCell = *nodePtrTemp;
                ...
                validTopBottomNode(validTopNodeNum, validBottomNodeNum,
                                   validTopNode, validBottomNode, iLayer,
                                   nodePtrTemp);

                if (numberOfNodeInCell == 3) {
                    insertTriangle3DCell(a_uGrid, validTopNodeNum,
                                         validBottomNodeNum, validTopNode,
                                         validBottomNode, nodePtrTemp,
                                         a_2DPointto3DPoints, iCell, iLayer);
                    // move pointer to next element
                    // nodePtrTemp +=
                    // (MeshConstants10::MAX_NUM_NODE_PER_CELL+1);
                } else if (numberOfNodeInCell == 4) {
                    insertQuad3DCell(a_uGrid, validTopNodeNum,
                                     validBottomNodeNum, validTopNode,
                                     validBottomNode, nodePtrTemp,
                                     a_2DPointto3DPoints, iCell, iLayer);
                    // nodePtrTemp +=
    ...
    debug1 << " tri_wedge " << m_tri_wedge << " tri_pyramid " << m_tri_pyramid
           << " tri_tetra " << m_tri_tetra;

    debug1 << " quad_hexhedron " << m_quad_hexhedron << " quad_wedge "
           << m_quad_wedge << " quad_pyramid " << m_quad_pyramid << "\n";
}
...
void avtSCHISMFileFormatImpl10::create2DPointMesh(vtkUnstructuredGrid* a_uGrid,
    ...
    if (!m_external_mesh_provider->fillSideCenterCoord2D(pointPtr,
                                                         a_timeState)) {
        stringstream msgStream(stringstream::out);
        msgStream << "Fail to retrieve edge center coord at step "
                  << a_timeState;
        EXCEPTION3(DBYieldedNoDataException, m_data_file, m_plugin_name,
                   msgStream.str());
    }
...
void avtSCHISMFileFormatImpl10::create3DPointMesh(vtkUnstructuredGrid* a_uGrid,
    ...
    if (!m_external_mesh_provider->fillSideCenterCoord3D(pointPtr,
                                                         a_timeState)) {
        stringstream msgStream(stringstream::out);
        msgStream << "Fail to retrieve edge center coord at step "
                  << a_timeState;
...
// this is the mesh consisits of center at 3d prism side face, used for var like
// flux
void avtSCHISMFileFormatImpl10::create3DPointFaceMesh(
    vtkUnstructuredGrid* a_uGrid, long* a_meshEle, const int& a_timeState) {
    int numNodes =
        m_total_valid_3D_side - (m_external_mesh_provider->numberOfSide());
    vtkPoints* points = vtkPoints::New();
    points->SetNumberOfPoints(numNodes);
    float* pointPtr = (float*)points->GetVoidPointer(0);

    if (!m_external_mesh_provider->fillSideFaceCenterCoord3D(pointPtr,
                                                             a_timeState)) {
        stringstream msgStream(stringstream::out);
        msgStream << "Fail to retrieve edge center coord at step "
                  << a_timeState;
...
vtkDataSet* avtSCHISMFileFormatImpl10::GetMesh(int                  a_timeState,
                                               avtSCHISMFileFormat* a_avtFile,
    ...
    cachedMesh =
        (a_avtFile->get_cache())
            ->GetVTKObject(cacheMeshID.c_str(), avtVariableCache::DATASET_NAME,
                           a_timeState, domainID, material.c_str());
    ...
    if ((cachedMesh != NULL) && (m_mesh_is_static)) {
        ...
        debug1 << "time used in building mesh :" << endTicks - startTicks
               << endl;
        updateMeshZCoordinates(uGrid->GetPoints(), a_timeState, mesh_name);
        return uGrid;
    }
    debug1 << mesh_name << " not in cache/or not static. Load from data."
           << endl;
    ...
    if (!strcmp(mesh_name, m_mesh_2d.c_str())) {
        ...
        create3DUnstructuredMesh(uGrid, faceNodesPtr, m2DPointto3DPoints,
                                 a_timeState);
    ...
    (a_avtFile->get_cache())
        ->CacheVTKObject(cacheMeshID.c_str(), avtVariableCache::DATASET_NAME,
                         a_timeState, domainID, material.c_str(), uGrid);
...
void avtSCHISMFileFormatImpl10::insertPyramid(vtkUnstructuredGrid* a_uGrid,
    ...
    // }
    for (int i = 0; i < 3; i++) {
        if (((a_faceNodePtr[i + 1] - 1) != p1) &&
            ((a_faceNodePtr[i + 1] - 1) != p2)) {
    ...
    if (p3 == MeshConstants10::INVALID_NUM) {
        stringstream msgStream(stringstream::out);
        msgStream << "fail to get pyramid apex for cell " << a_Cell
                  << "on layer " << a_layerID;
        EXCEPTION1(InvalidVariableException, msgStream.str());
    }
...
void avtSCHISMFileFormatImpl10::insertTriangle3DCell(
    vtkUnstructuredGrid* a_uGrid,
    ...
    if ((a_validBottomNodeNum == 0) ||
        ((a_validBottomNodeNum == 2) &&
         (a_validTopNodeNum == 2))) // no 3d cell at all
    {
        return;
    }
    ...
    if (a_validBottomNodeNum == 3) // this is a wedge
    {
        ...
        for (int iNode = 3; iNode < 2 * 3; ++iNode) {
            long p = a_validTopNode[iNode - 3];
            verts[iNode] =
                a_2DPointto3DPoints[p * m_num_layers + a_layerID + 1];
        }
        a_uGrid->InsertNextCell(VTK_WEDGE, 2 * 3, verts);
        m_tri_wedge++;
    } else if (a_validBottomNodeNum ==
               2) // bottom have two node, this is a pyramid
    {
        vtkIdType verts[5];
...
bool avtSCHISMFileFormatImpl10::fourPointsCoplanar(double p1[3],
    ...
    double dotvalue = v1[0] * (v2[1] * v3[2] - v2[2] * v3[1]) -
...
void avtSCHISMFileFormatImpl10::insert8NodesPolyhedron(
    vtkUnstructuredGrid* a_uGrid,
    vtkIdType*           a_verts,
    long*                a_validTopNode,
    long*                a_validBottomNode,
    ...
    debug1 << "8 nodes polyhedron at layer " << a_layerID << " cell " << a_Cell
           << " " << a_bottomCoplane << " " << a_topCoplane << "\n";
    ...
    for (int inode = 0; inode < 4; inode++) {
        ...
        if ((a_layerID + 2) >= bottomLayer) {
            p_t[inode] =
                a_2DPointto3DPoints[v_t * m_num_layers + a_layerID + 1];
        } else {
            p_t[inode] =
                a_2DPointto3DPoints[v_t * m_num_layers + bottomLayer - 1];
        }
    }
    ...
    if (!a_bottomCoplane) {
        ...
        for (int iNode = 0; iNode < 3; iNode++) {
            ...
            faceBottom1[iNode] =
                a_2DPointto3DPoints[p1 * m_num_layers + a_layerID];
        ...
        faceBottom2[2] =
            a_2DPointto3DPoints[a_validBottomNode[0] * m_num_layers +
                                a_layerID];
    ...
    if (!a_topCoplane) {
        ...
        for (int iNode = 0; iNode < 3; iNode++) {
            ...
            // faceTop1[iNode]=
            // a_2DPointto3DPoints[p1*m_num_layers+a_layerID+1];
        ...
        // faceTop2[2]=
        // a_2DPointto3DPoints[a_validTopNode[0]*m_num_layers+a_layerID+1];
        faceTop2[2] = p_t[0];
...
void avtSCHISMFileFormatImpl10::insert7NodesPolyhedron(
    vtkUnstructuredGrid* a_uGrid,
    vtkIdType*           a_verts,
    long*                a_validTopNode,
    long*                a_validBottomNode,
    ...
    // debug1<<"degenerated node: "<<degeneratedNode<<" at
    // "<<degeneratedNodeLoc<<" \n";
    ...
    vtkIdType faceCut[3];
    // faceCut[0]=
    // a_2DPointto3DPoints[degeneratedNode*m_num_layers+a_layerID+1];
    ...
    faceCut[1] =
        a_2DPointto3DPoints[a_validTopNode[backNodeLoc] * m_num_layers +
                            a_layerID];
    faceCut[2] = a_2DPointto3DPoints[a_validTopNode[forNodeLoc] * m_num_layers +
                                     a_layerID];
...
void avtSCHISMFileFormatImpl10::insertQuad3DCell(
    vtkUnstructuredGrid* a_uGrid,
    const int&           a_validTopNodeNum,
    const int&           a_validBottomNodeNum,
    long*                a_validTopNode,
...

{
    ...
    // debug1<< "valid top 3d node "<<p_t[0]<<" "<<p_t[1]<<" "<<p_t[2]<<"
    // "<<p_t[3]<<"\n";
    ...
    bool topCoplane = fourPointsCoplanar(coord1, coord2, coord3, coord4);
    ...
    if ((a_validBottomNodeNum == 4)) {
        ...
        bool bottomCoplane = fourPointsCoplanar(coord1, coord2, coord3, coord4);
        ...
        // debug1<<"hexahedron at level (top and bottom coplane) "
        // <<a_layerID<<" cell "<<a_Cell<<endl;
        m_quad_hexhedron++;
    } else if (a_validBottomNodeNum ==
               3) // still do it using hexahedron by using
                  // top point of degenerated point twice
    {
        ...
        m_quad_hexhedron++;
               ...
               2) // bottom have two node, this is a WEDGE
    {
...
void avtSCHISMFileFormatImpl10::loadAndCacheZSide(const int& a_timeState,
                                                  float*     a_sideCenterZPtr) {
    if (!m_external_mesh_provider->zSideCenter3D(a_sideCenterZPtr,
                                                 a_timeState)) {
        stringstream msgStream(stringstream::out);
...
void avtSCHISMFileFormatImpl10::loadAndCacheZEle(const int& a_timeState,
                                                 float*     a_eleCenterZPtr) {
...
void avtSCHISMFileFormatImpl10::loadAndCacheZ(const int& a_timeState,
                                              float*     a_nodeZPtr) {
...
void avtSCHISMFileFormatImpl10::load_node_dry_wet(const int& a_time) {
    ...
    if (m_node_dry_wet_cached_time == a_time) {
        ...
        m_external_mesh_provider->fill_node_dry_wet(m_node_dry_wet,
                                                    m_ele_dry_wet);
        ...
        m_nominal_size_per_layer[MeshConstants10::NODE] =
            nominal_data_size_per_layer;
...
void avtSCHISMFileFormatImpl10::load_ele_dry_wet(const int& a_time) {
    ...
    if (m_ele_dry_wet_cached_time == a_time) {
        ...
        /* std::string SCHISMVarName = MeshConstants10::ELEM_DRYWET;
         ...
         {
                 EXCEPTION1(InvalidVariableException, ("no "+SCHISMVarName+" in
         "+m_data_file_ptr->file()).c_str());
         }
          ...
          {
                  stringstream msgStream(stringstream::out);
              msgStream <<"Fail to retrieve "<<SCHISMVarName << " at step "
         <<a_time;
        ...
        m_nominal_size_per_layer[MeshConstants10::ELEM] =
            nominal_data_size_per_layer;
...
void avtSCHISMFileFormatImpl10::load_side_dry_wet(const int& a_time) {
    ...
    if (m_side_dry_wet_cached_time == a_time) {
        ...
        m_external_mesh_provider->fill_side_dry_wet(m_side_dry_wet,
                                                    m_ele_dry_wet);
        ...
        m_nominal_size_per_layer[MeshConstants10::EDGE] =
            nominal_data_size_per_layer;
...
void avtSCHISMFileFormatImpl10::getMeshDimensions(
    MeshProvider10* a_meshProviderPtr) {
    ...
    if (m_num_mesh_nodes < 0) // to do: face centered data will get num faces
    {
        EXCEPTION1(InvalidVariableException,
                   ("no nodes found in " + a_meshProviderPtr->file()).c_str());
    }
    ...
    if (m_num_mesh_faces < 0) {
        EXCEPTION1(
            ...
            ("no element found in " + a_meshProviderPtr->file()).c_str());
    }
    ...
    if (m_num_mesh_edges < 0) {
        EXCEPTION1(InvalidVariableException,
                   ("no edges found in " + a_meshProviderPtr->file()).c_str());
    }
    ...
    if (m_num_layers < 0) {
        EXCEPTION1(InvalidVariableException,
                   ("no layer found ,one required at least, in " +
                    a_meshProviderPtr->file())
                       .c_str());
    }
...
void avtSCHISMFileFormatImpl10::loadMeshCoordinates(
    MeshProvider10* a_meshProviderPtr) {
...
vtkDataArray*
avtSCHISMFileFormatImpl10::getBottom(const string& a_bottom_name) {
    int*        kbp00;
    std::string data_center(MeshConstants10::NODE);
    long        nominal_num_data_per_Layer = 0;
...
vtkDataArray* avtSCHISMFileFormatImpl10::GetVar(int         a_timeState,
    ...
    // if (m_scribeIO && ((!strcmp(a_varName,
    // MeshConstants10::NODE_SURFACE_LABEL.c_str())) || (!strcmp(a_varName,
    // m_node_depth_label.c_str()))))
    //{
    //
    //  SCHISMVarPtr =
    //  m_external_mesh_provider->get_mesh_data_ptr()->get_var(SCHISMVarName);
    //}
    // else
    //{
    SCHISMVarPtr = m_data_file_ptr->get_var(SCHISMVarName);
    ...
    int* drywet;
    debug1 << "getting " << a_varName << " " << level_center << " "
           << data_center << "\n";
    ...
    if ((!strcmp(a_varName, m_node_depth_label.c_str())) ||
        (!strcmp(a_varName, MeshConstants10::NODE_SURFACE_LABEL.c_str())) ||
        ...
        getSingleLayerVar(valBuff, m_data_file_ptr, a_timeState, SCHISMVarName);
        ...
        for (int iNode = 0; iNode < numData; iNode++) {
            ...
            // || (!strcmp(a_varName, MeshConstants10::NODE_BOTTOM2.c_str())))
            //{
    ...
    if (!m_scribeIO) {
        if (!(SCHISMVarPtr->get(valBuff))) {
            stringstream msgStream(stringstream::out);
            msgStream << "Fail to retrieve " << a_varName << " at step "
                      << a_timeState;
            EXCEPTION3(DBYieldedNoDataException, m_data_file, m_plugin_name,
                       msgStream.str());
        }
    ...
    if (varName.find(m_depth_average_suffix) == string::npos) {
        ...
        if (is_bottom || is_surface) // for surface and bottom data, same as
                                     // number of 2D node
        {
            ntuples = numDataPerLayer;
        }
        if (is_half_layer)
            ntuples -= half_remove_cell;
        rv->SetNumberOfTuples(ntuples);
...
vtkDataArray* avtSCHISMFileFormatImpl10::GetVector2d(int         a_timeState,
    ...
    SCHISMVar10* SCHISMVarPtr =
        m_external_mesh_provider->get_mesh_data_ptr()->get_var(xcom);
    SCHISMVar10* SCHISMVarPtr2 =
        m_external_mesh_provider->get_mesh_data_ptr()->get_var(ycom);
    long numDataPerLayer = m_num_mesh_nodes;
    ...
    valBuff = new float[numData];
    getSingleLayerVar(valBuff, m_external_mesh_provider->get_mesh_data_ptr(),
                      a_timeState, xcom);
    float* valBuff2 = NULL;

    valBuff2 = new float[numData];
    getSingleLayerVar(valBuff2, m_external_mesh_provider->get_mesh_data_ptr(),
                      a_timeState, ycom);
...
vtkDataArray* avtSCHISMFileFormatImpl10::GetVectorVar(int         a_timeState,
                                                      const char* a_varName) {
    ...
    if (found != std::string::npos) {
        x_com_name = varName.substr(0, found) + "X" +
                     varName.substr(found, varName.length());
    } else {
    ...
    if (varNames.size() == 2) {
        if (m_data_file_ptr2) {
            ...
            stringstream msgStream(stringstream::out);
            msgStream << " " << a_varName
                      << " miss another component data source file \n";
    ...
    if (!SCHISMVarIs3D(SCHISMVarPtr)) {
        ...
        valBuff = new float[numData];
        getSingleLayerVar(valBuff, m_data_file_ptr, a_timeState, varNames[0]);
        float* valBuff2 = NULL;

        if (SCHISMVarPtr2) {
            valBuff2 = new float[numData];
            getSingleLayerVar(valBuff2, m_data_file_ptr2, a_timeState,
                              varNames[1]);
        }
    ...
    if (SCHISMVarPtr2) {
        ...
        if (m_scribeIO) {
            if (!(SCHISMVarPtr2->get(valBuff2, bottom))) {
                stringstream msgStream(stringstream::out);
                msgStream << "Fail to retrieve vector " << a_varName
                          << "another component at step " << a_timeState;
                EXCEPTION3(DBYieldedNoDataException, m_data_file, m_plugin_name,
                           msgStream.str());
            }
        } else {
        ...
        for (long iNode = 0; iNode < totalNumLayers * numDataPerLayer;
             iNode++) {
            valBuffAll[ncomps * iNode] = valBuff[iNode];
            valBuffAll[ncomps * iNode + 1] = valBuff2[iNode];
        }
    ...
    for (int iNode = 0; iNode < numDataPerLayer; iNode++) {
        node_start_index[iNode] = valid_var_size;
        valid_var_size +=
            ((num_layers - std::max(1, kbp00[iNode]) + 1) * ncomps);
    ...
    if (varName.find(m_depth_average_suffix) == string::npos) {
        ...
        debug1 << "totla number of vect tuples " << ntuples << " "
               << num_data_layers << " \n";
        ...
        for (int iLayer = 0; iLayer < num_data_layers; iLayer++) {
            ...
            for (int iNode = 0; iNode < numDataPerLayer; iNode++) {
                ...
                if (layer >= bottom_layer) {
                    // if(!drywet[iNode])
                    if ((!drywet[iNode]) ||
                        (drywet[iNode] && (!(m_dry_wet_flag)))) {

                        for (int iComp = 0; iComp < ncomps; iComp++) {
                            oneEntry[iComp] =
                                valBuffAll[start_index +
                                           (layer + 1 -
                                            std::max(1, kbp00[iNode])) *
                                               ncomps +
                                           iComp];
                        }
        ...
        vectorDepthAverage(averageState, valBuffAll, node_start_index,
                           a_timeState, ncomps, data_center, level_center);
...
void avtSCHISMFileFormatImpl10::prepare_average(
    int*&              a_kbp,
    int*&              a_mapper,
    float*&            a_zPtr,
    const int&         a_timeState,
...
//
// ****************************************************************************
void avtSCHISMFileFormatImpl10::depthAverage(
    float*             a_averageState,
    ...
    prepare_average(kbp, mapper, zPtr, a_timeState, a_horizontal_center,
                    a_vertical_center);
    ...
    for (int iNode = 0; iNode < numData; iNode++) {
        if (kbp[iNode] <= m_num_layers) {
            ...
            int num_valid_data_layers =
                m_num_layers - std::max(1, kbp[iNode]) + 1;
            ...
            if (num_valid_data_layers < 1) {
                stringstream msgStream(stringstream::out);
                msgStream << "less than one layer at node" << iNode
                          << " when averaging\n";
                EXCEPTION3(DBYieldedNoDataException, m_data_file, m_plugin_name,
                           msgStream.str());
            }
            ...
            float* zCoords =
                new float[m_num_layers - std::max(1, kbp[iNode]) +
                          1]; // take all the valid zs for this node
            ...
            if (kbp[iNode] == 0) {
                ...
                for (int ilevel = 0; ilevel < num_valid_z_layers - 1;
                     ilevel++) {
                    if (zCoords[ilevel + 1] < zCoords[ilevel]) {
                        last_level_be_removed = ilevel;
                        // debug1<<iNode<<" has done inactive z removal to
                        // "<<last_level_be_removed<<"\n";
                        break;
                    }
                }
                num_valid_z_layers =
                    num_valid_z_layers - last_level_be_removed - 1;
                ...
                for (int ilevel = 0; ilevel < num_valid_z_layers; ilevel++) {
                    ...
                    zCoords[ilevel] =
                        zCoords[ilevel + last_level_be_removed + 1];
                }
            }
            ...
            if (a_vertical_center != MeshConstants10::HALF_LAYER) {
                ...
                averageState =
                    trapezoidAverage(states, zCoords, num_valid_data_layers);
            } else {
                averageState = rectAverage(states, zCoords, num_valid_z_layers);
            }
...
//
// ****************************************************************************
void avtSCHISMFileFormatImpl10::vectorDepthAverage(
    float*             a_averageState,
    float*             a_layeredState,
    long*              a_nodeDataStart,
    const int&         a_timeState,
    ...
    for (int iNode = 0; iNode < numData; iNode++) {
        if (kbp[iNode] <= m_num_layers) {
            ...
            if (num_valid_data_layers < 1) {
                stringstream msgStream(stringstream::out);
                msgStream << "less than one level at node" << iNode
                          << " when averaging\n";
                EXCEPTION3(DBYieldedNoDataException, m_data_file, m_plugin_name,
                           msgStream.str());
            }
            ...
            for (int iLayer = 0; iLayer < num_valid_data_layers; iLayer++) {
                for (int iComp = 0; iComp < a_ncomps; iComp++) {
                    states[iComp][iLayer] =
                        a_layeredState[iComp + iLayer * a_ncomps + startid];
                    if (iNode == 1850) {
            ...
            float* zCoords = new float[m_num_layers - std::max(1, kbp[iNode]) +
                                       1]; // take all the zs for the mesh
            ...
            if (kbp[iNode] == 0) {
                ...
                for (int ilevel = 0; ilevel < num_valid_z_layers; ilevel++) {
                    for (int iComp = 0; iComp < a_ncomps; iComp++) {
                        states[iComp][ilevel] =
                            states[iComp][ilevel + last_level_be_removed + 1];
                    }
                    zCoords[ilevel] =
                        zCoords[ilevel + last_level_be_removed + 1];
                }
            ...
            for (int iComp = 0; iComp < a_ncomps; iComp++) {
                float averageState;
                if (a_vertical_center != MeshConstants10::HALF_LAYER) {
                    averageState = trapezoidAverage(states[iComp], zCoords,
                                                    num_valid_data_layers);
                } else {
                    averageState =
                        rectAverage(states[iComp], zCoords, num_valid_z_layers);
                    if (iNode == 1850) {
                        debug1 << averageState << " ";
                    }
                }
            ...
            for (int iComp = 0; iComp < a_ncomps; iComp++) {
                a_averageState[iNode * a_ncomps + iComp] =
                    MeshConstants10::DRY_STATE;
            }
        }
    }
...
void avtSCHISMFileFormatImpl10::getSingleLayerVar(
    float*             a_valBuff,
    ...
    time_t endTicks = clock();
    debug1 << "time used in getting var " << a_varName << ":"
           << endTicks - startTicks << endl;
}
...
void avtSCHISMFileFormatImpl10::ActivateTimestep(
...
void avtSCHISMFileFormatImpl10::Initialize(std::string a_data_file) {
    ...
    if (!m_initialized) {
        ...
        try {
            ...
            if (itX != Vector3DVarMapX.end()) {
                ...
                std::string extra_file =
                    m_data_file_path + "\\" +
                    file_name.replace(found_underline - 1, 1, "Y");
#else
                std::string extra_file =
                    m_data_file_path + "/" +
                    file_name.replace(found_underline - 1, 1, "Y");
                ...
                std::string extra_file =
                    m_data_file_path + "\\" +
                    file_name.replace(found_underline - 1, 1, "X");
#else
                std::string extra_file =
                    m_data_file_path + "/" +
                    file_name.replace(found_underline - 1, 1, "X");
        ...
        // if "V10" in the string 5.8 format, otherwise scribeIo format (no
        // atribute or other values)
        try {
            std::string source =
                m_data_file_ptr->global_att_as_string(MeshConstants10::source);
            std::size_t found_v10 =
                source.find(MeshConstants10::SCHISM58_OUTPUT_FORMAT);
        ...
        // here a simple and temp way to decided if data file is the latest
        // scriber format
        std::string meshFilePath = m_data_file;
        // std::size_t found2 = m_data_file.find("schout");

        // if not name by schout then treated as scriber format, mesh file
        // should be out2d*.nc + zcor*.nc if (found2==std::string::npos)
        if (m_scribeIO == true) {
            // m_scribeIO = true;
        ...
        if (!okay) {
            EXCEPTION1(InvalidDBTypeException,
                       (m_data_file_path + "is not a valid SCHISM output file")
                           .c_str());
        }
        ...
        if (!okay) {
            EXCEPTION1(
                ...
                (m_data_file_path + "doesn't contain valid mesh infomation")
                    .c_str());
        }
