...
SCHISMMeshProvider10::SCHISMMeshProvider10(const std::string& a_fileHasMeshData)
    : MeshProvider10(a_fileHasMeshData),
      m_kbp00(NULL),
      m_layerSCoords(NULL),
      m_kbs(NULL),
      m_kbe(NULL),
...
bool SCHISMMeshProvider10::fillKbp00(int*       a_cache,
                                     const int& a_timeStep) const {
...
bool SCHISMMeshProvider10::loadSide() {
    ...
    if (m_dataFilePtr->inquire_var(MeshConstants10::EDGE_NODE)) {
        SCHISMVar10* side_node_ptr =
            m_dataFilePtr->get_var(MeshConstants10::EDGE_NODE);

        if (!(side_node_ptr->get(m_sideNodes))) {
            throw SCHISMFileException10(
                "fail to retrieve dim " + MeshConstants10::EDGE_NODE +
                " from data file " + m_dataFilePtr->file());
            return false;
        }
    ...
    {
        long* mesh_nodes =
            new long[m_number_element *
                     (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1)];
        this->fillMeshElement(mesh_nodes);
        meshSideNode(m_sideNodes, mesh_nodes, m_number_side, m_number_element,
                     m_number_node);
        delete mesh_nodes;
        return true;
    }
}
void SCHISMMeshProvider10::fill_ele_dry_wet(int*&      a_ele_dry_wet,
                                            const int& a_step) {
    ...
    if (!(m_dataFilePtr->inquire_var(SCHISMVarName))) {
        ...
        if (!(m_dataFilePtr->inquire_var(SCHISMVarName))) {
            throw SCHISMFileException10("no ele dry wet flag in " +
                                        m_dataFilePtr->file());
        }
    }
    ...
    try {
        ...
        throw SCHISMFileException10("no " + SCHISMVarName + " in " +
                                    m_dataFilePtr->file());
    }
...
void SCHISMMeshProvider10::fill_node_dry_wet(int*& a_node_dry_wet,
                                             int*  a_ele_dry_wet) {

    long* mesh_nodes = new long[m_number_element *
                                (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1)];
    this->fillMeshElement(mesh_nodes);
    ...
    if (!m_node_neighbor_ele_filled) {
        ...
        meshSideNodeNeiborEle(mesh_nodes, m_node_neighbor_ele,
                              m_side_neighbor_ele, max_ele_at_node,
                              m_number_element, m_number_node, m_number_side);
    ...
    for (long inode = 0; inode < m_number_node; inode++) {
        ...
        for (int iele = 0; iele < m_max_ele_at_node; iele++) {
            long ele_global_id =
                m_node_neighbor_ele[m_max_ele_at_node * inode + iele];
...
void SCHISMMeshProvider10::fill_side_dry_wet(int*& a_side_dry_wet,
                                             int*  a_ele_dry_wet) {
    long* mesh_nodes = new long[m_number_element *
                                (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1)];
    this->fillMeshElement(mesh_nodes);
    ...
    for (long iside = 0; iside < m_number_side; iside++) {
        ...
        for (int iele = 0; iele < num_side_neighbor_ele; iele++) {
            long ele_global_id =
                m_side_neighbor_ele[num_side_neighbor_ele * iside + iele];
            if (!(ele_global_id == invalid_id)) {
                if (a_ele_dry_wet[ele_global_id] == MeshConstants10::WET_FLAG) {
                    all_ele_dry = false;
                    break;
...
bool SCHISMMeshProvider10::loadMeshDim() {
    ...
    SCHISMDim10* dimNodePtr =
        m_dataFilePtr->get_dim(MeshConstants10::DIM_MESH_NODES);

    if (dimNodePtr->is_valid()) {
        m_number_node = dimNodePtr->size();
    } else {
        m_number_node = MeshConstants10::INVALID_NUM;
        throw SCHISMFileException10("fail to retrieve dim " +
                                    MeshConstants10::DIM_MESH_NODES +
                                    " from data file " + m_dataFilePtr->file());
        return false;
    }

    // SELFEDim constructor and destructor are private, no need to del.
    SCHISMDim10* dimFacePtr =
        m_dataFilePtr->get_dim(MeshConstants10::DIM_MESH_FACES);
    ...
    if (dimFacePtr->is_valid()) {
        ...
        throw SCHISMFileException10("fail to retrieve dim " +
                                    MeshConstants10::DIM_MESH_FACES +
                                    " from data file " + m_dataFilePtr->file());
        return false;
    }

    // SELFEDim constructor and destructor are private, no need to del.
    SCHISMDim10* dimSidePtr =
        m_dataFilePtr->get_dim(MeshConstants10::DIM_MESH_EDGES);
    ...
    if (dimSidePtr->is_valid()) {
        ...
        throw SCHISMFileException10("fail to retrieve dim " +
                                    MeshConstants10::DIM_MESH_EDGES +
                                    " from data file " + m_dataFilePtr->file());
        return false;
    }

    SCHISMDim10* dimLayerPtr =
        m_dataFilePtr->get_dim(MeshConstants10::DIM_LAYERS);
    ...
    if (m_dataFilePtr->inquire_var(MeshConstants10::LAYER_SCOORD)) {
        SCHISMVar10* sVarPtr =
            m_dataFilePtr->get_var(MeshConstants10::LAYER_SCOORD);

        if (!(sVarPtr->is_valid())) {
            throw SCHISMFileException10(
                "fail to retrieve dim " + MeshConstants10::LAYER_SCOORD +
                " from data file " + m_dataFilePtr->file());
            return false;
        } else {
            ...
            if (!(hsAttPtr->is_valid())) {
                throw SCHISMFileException10(
                    "fail to retrieve dim " + MeshConstants10::HS +
                    " from data file " + m_dataFilePtr->file());
                return false;
            }
            ...
            if (!(hcAttPtr->is_valid())) {
                throw SCHISMFileException10(
                    "fail to retrieve dim " + MeshConstants10::HC +
                    " from data file " + m_dataFilePtr->file());
                return false;
            }
            m_hc = hcAttPtr->float_value(0);

            SCHISMAtt10* thetabAttPtr =
                sVarPtr->get_att(MeshConstants10::THETAB);
            if (!(thetabAttPtr->is_valid())) {
                throw SCHISMFileException10(
                    "fail to retrieve dim " + MeshConstants10::THETAB +
                    " from data file " + m_dataFilePtr->file());
                return false;
            }
            m_thetab = thetabAttPtr->float_value(0);

            SCHISMAtt10* thetafAttPtr =
                sVarPtr->get_att(MeshConstants10::THETAF);
            if (!(thetafAttPtr->is_valid())) {
                throw SCHISMFileException10(
                    "fail to retrieve dim " + MeshConstants10::THETAF +
                    " from data file " + m_dataFilePtr->file());
                return false;
            }
            ...
            if (!sVarPtr->get(stemp)) {
                throw SCHISMFileException10(
                    "fail to retrieve var " + MeshConstants10::LAYER_SCOORD +
                    " from data file " + m_dataFilePtr->file());
            }

            int sigma_layer_start_id = m_number_layer - num_sigma_layers;
            int itemp = 0;
            for (int iLayer = sigma_layer_start_id; iLayer < m_number_layer;
                 iLayer++) {
...
// z is filled with 0
bool SCHISMMeshProvider10::fillPointCoord2D(float*     a_pointCoord,
                                            const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        ...
        retrieve1DVar(nodeXPtr, m_dataFilePtr, MeshConstants10::NODE_X,
                      m_number_node);

        retrieve1DVar(nodeYPtr, m_dataFilePtr, MeshConstants10::NODE_Y,
                      m_number_node);
...
bool SCHISMMeshProvider10::fillPointCoord2D(double*    a_pointCoord,
                                            const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
bool SCHISMMeshProvider10::fillSideCenterCoord3D(float*     a_pointCoord,
                                                 const int& a_timeStep) const {
    // if( m_mesh_loaded==false)
    //{
    //    return false;
...
bool SCHISMMeshProvider10::fillSideCenterCoord3D(double*    a_pointCoord,
                                                 const int& a_timeStep) const {
    // if( m_mesh_loaded==false)
    //{
    //    return false;
...
bool SCHISMMeshProvider10::fillSideFaceCenterCoord3D(
    float* a_pointCoord, const int& a_timeStep) const {
    /*if( m_mesh_loaded==false)
    {
       return false;
    }
...
bool SCHISMMeshProvider10::fillSideFaceCenterCoord3D(
    double* a_pointCoord, const int& a_timeStep) const {
    /*if( m_mesh_loaded==false)
    {
       return false;
    }
...
// z is filled with 0
bool SCHISMMeshProvider10::fillSideCenterCoord2D(float*     a_pointCoord,
                                                 const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
// z is filled with 0
bool SCHISMMeshProvider10::fillSideCenterCoord2D(double*    a_pointCoord,
                                                 const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
bool SCHISMMeshProvider10::fillEleCenterCoord3D(float*     a_pointCoord,
                                                const int& a_timeStep) const {
    /*if( m_mesh_loaded==false)
    {
       return false;
    ...
    long* meshElementNodes =
        new long[(MeshConstants10::MAX_NUM_NODE_PER_CELL + 1) *
                 m_number_element];
        ...
        for (long iEle = 0; iEle < m_number_element; iEle++) {
            ...
            if (iLayer >= (std::max(1, kbe[iEle]) - 1)) {
                ...
                for (int iNode = 0; iNode < numNode; iNode++) {
                    long node = meshElementNodes[iEle * (max_node_in_cell + 1) +
                                                 1 + iNode] -
                                1;
...
bool SCHISMMeshProvider10::fillEleCenterCoord3D(double*    a_pointCoord,
                                                const int& a_timeStep) const {
    /*if( m_mesh_loaded==false)
    {
       return false;
...
// z is filled with 0
bool SCHISMMeshProvider10::fillEleCenterCoord2D(float*     a_pointCoord,
                                                const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
        ...
        for (long iEle = 0; iEle < m_number_element; iEle++) {
            ...
            for (int iNode = 0; iNode < numNode; iNode++) {
                long node = meshElementNodes[iEle * (max_node_in_cell + 1) + 1 +
                                             iNode] -
                            1;
                x_sum += nodeXPtr[iNode];
                y_sum += nodeYPtr[iNode];
...
// z is filled with 0
bool SCHISMMeshProvider10::fillEleCenterCoord2D(double*    a_pointCoord,
                                                const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
bool SCHISMMeshProvider10::fillPointCoord3D(float*     a_pointCoord,
                                            const int& a_timeStep) const {
    /*if( m_mesh_loaded==false)
    {
       return false;
...
bool SCHISMMeshProvider10::fillPointCoord3D(double*    a_pointCoord,
                                            const int& a_timeStep) const {
    /*if( m_mesh_loaded==false)
    {
       return false;
...
bool SCHISMMeshProvider10::fillMeshElement(long* a_elementCache) const {
    if (!m_mesh_loaded) {
        ...
        SCHISMVar10* ncFacePtr =
            m_dataFilePtr->get_var(MeshConstants10::MESH_FACE_NODES);
        if (!(ncFacePtr->is_valid())) {

            throw SCHISMFileException10("no mesh element in " +
                                        m_dataFilePtr->file());
        }
...
bool SCHISMMeshProvider10::zcoords2D(float*     a_zCachePtr,
                                     const int& a_timeStep) const {
    for (long iNode = 0; iNode < m_number_node; iNode++) {
...
bool SCHISMMeshProvider10::zcoords3D(float*     a_zCachePtr,
                                     const int& a_timeStep) const {

    return false;
}

bool SCHISMMeshProvider10::zcoords3D2(float*     a_zCachePtr,
                                      const int& a_timeStep) const {

    return false;
}

bool SCHISMMeshProvider10::zSideCenter2D(float*     a_zCachePtr,
                                         const int& a_timeStep) const {
...
bool SCHISMMeshProvider10::zSideCenter3D(float*     a_zCachePtr,
                                         const int& a_timeStep) const {
    float* zPtr = new float[m_number_layer * m_number_node];
    float* zPtrTemp = zPtr;
    zcoords3D2(zPtr, a_timeStep);
...
bool SCHISMMeshProvider10::zEleCenter2D(float*     a_zCachePtr,
                                        const int& a_timeStep) const {
...
bool SCHISMMeshProvider10::zEleCenter3D(float*     a_zCachePtr,
                                        const int& a_timeStep) const {
    float* zPtr = new float[m_number_layer * m_number_node];
    float* zPtrTemp = zPtr;
    zcoords3D2(zPtr, a_timeStep);
...
bool SCHISMMeshProvider10::depth(float* a_cache) const { return false; }

bool SCHISMMeshProvider10::mesh3d_is_static() const {
    int dynamic_flag =
        m_dataFilePtr->global_att_as_int(MeshConstants10::DYNAMIC_MESH);
    if (!dynamic_flag) {
        return true;
    } else
        return false;
