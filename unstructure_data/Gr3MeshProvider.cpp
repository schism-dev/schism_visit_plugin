...
Gr3MeshProvider::Gr3MeshProvider(const std::string& a_gr3File)
    : MeshProvider(a_gr3File),
      m_nodeXPtr(NULL),
      m_nodeYPtr(NULL),
      m_faceNodesPtr(NULL),
      m_depthPtr(NULL)
...
bool Gr3MeshProvider::provide3DMesh() const { return false; }
void Gr3MeshProvider::loadMesh() {
    ...
    m_faceNodesPtr =
        new int[(MeshConstants::MAX_NUM_NODE_PER_CELL + 1) * numMeshFaces];
    m_depthPtr = new float[numMeshNodes];

    for (int iNode = 0; iNode < numMeshNodes; iNode++) {
        int valtemp;
        gr3FileStream >> valtemp >> m_nodeXPtr[iNode] >> m_nodeYPtr[iNode] >>
            m_depthPtr[iNode];
    }
    ...
    for (int iFace = 0; iFace < numMeshFaces; iFace++) {
        ...
        for (int iNode = (numNode + 1);
             iNode < (MeshConstants::MAX_NUM_NODE_PER_CELL + 1); iNode++) {
    ...
    m_number_side =
        meshSideNum(m_faceNodesPtr, m_number_element, m_number_node);

    m_sideNodes = new int[m_number_side * 2];
    meshSideNode(m_sideNodes, m_faceNodesPtr, m_number_side, m_number_element,
                 m_number_node);
...
// z is filled with 0
bool Gr3MeshProvider::fillPointCoord2D(float*     a_pointCoord,
...
bool Gr3MeshProvider::fillPointCoord3D(float*     a_pointCoord,
                                       const int& a_timeStep) const {
...
bool Gr3MeshProvider::fillMeshElement(int* a_elementCache) const {
    ...
    for (int i = 0;
         i < (MeshConstants::MAX_NUM_NODE_PER_CELL + 1) * m_number_element;
         i++) {
...
bool Gr3MeshProvider::zcoords2D(float*     a_zCachePtr,
                                const int& a_timeStep) const {
    for (int iNode = 0; iNode < m_number_node; iNode++) {
...
bool Gr3MeshProvider::fillSideCenterCoord2D(float*     a_pointCoord,
                                            const int& a_timeStep) const {
...
bool Gr3MeshProvider::fillEleCenterCoord2D(float*     a_pointCoord,
                                           const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
        ...
        for (int iEle = 0; iEle < m_number_element; iEle++) {
            ...
            for (int iNode = 0; iNode < numNode; iNode++) {
                int node =
                    m_faceNodesPtr[iEle * (maxNodeInCell + 1) + 1 + iNode] - 1;
...
bool Gr3MeshProvider::zSideCenter2D(float*     a_zCachePtr,
                                    const int& a_timeStep) const {
...
bool Gr3MeshProvider::zEleCenter2D(float*     a_zCachePtr,
                                   const int& a_timeStep) const {
    for (int iNode = 0; iNode < m_number_side; iNode++) {
...
bool Gr3MeshProvider::zcoords3D(float*     a_zCachePtr,
                                const int& a_timeStep) const {
    return false;
}

bool Gr3MeshProvider::slayers(float* a_cache) const { return false; }
