...
class MDSCHISMMeshProvider : public MeshProvider10 {
  public:
    MDSCHISMMeshProvider(const std::string& a_ncfile,
                         SCHISMFile10*      a_nc_ptr,
                         const std::string& a_local_gloabl_file);
    ...
    bool fillSideCenterCoord2D(float*     a_pointCoord,
                               const int& a_timeStep) const;

    // provides side centers x,y, z at a time step
    bool fillSideCenterCoord3D(float*     a_pointCoord,
                               const int& a_timeStep) const;

    // provides sideface centers x,y, z at a time step
    bool fillSideFaceCenterCoord3D(float*     a_pointCoord,
                                   const int& a_timeStep) const;
    ...
    // provides side centers x,y, z at a time step
    bool fillSideCenterCoord2D(double*    a_pointCoord,
                               const int& a_timeStep) const;

    // provides side centers x,y, z at a time step
    bool fillSideCenterCoord3D(double*    a_pointCoord,
                               const int& a_timeStep) const;

    // provides sideface centers x,y, z at a time step
    bool fillSideFaceCenterCoord3D(double*    a_pointCoord,
                                   const int& a_timeStep) const;

    bool fillEleCenterCoord2D(double*    a_pointCoord,
                              const int& a_timeStep) const;

    // provides side centers x,y, z at a time step
    bool fillEleCenterCoord3D(double*    a_pointCoord,
                              const int& a_timeStep) const;
