...
MDSCHISMMeshProvider::MDSCHISMMeshProvider(const std::string& a_ncfile,
                                           SCHISMFile10*      a_nc_ptr,
                                           const std::string& a_local_file)
    : MeshProvider10(a_ncfile),
      m_kbp00(NULL),
      m_layerSCoords(NULL),
      m_kbs(NULL),
      m_kbe(NULL),
...
bool MDSCHISMMeshProvider::fillKbp00(int*       a_cache,
...
void MDSCHISMMeshProvider::fill_node_dry_wet(int*& a_node_dry_wet,
                                             int*  a_ele_dry_wet) {
...
void MDSCHISMMeshProvider::fill_side_dry_wet(int*& a_side_dry_wet,
                                             int*  a_ele_dry_wet) {
    long* mesh_nodes = new long[m_number_element *
                                (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1)];
    this->fillMeshElement(mesh_nodes);
...
bool MDSCHISMMeshProvider::loadMesh() {
    ...
    *(localFileStream) >> m_number_node;
    ...
    for (long iNode = 0; iNode < m_number_node; iNode++) {
        long v1, v2;
        *(localFileStream) >> v1 >> v2;
        m_local_node_id_to_global_id[iNode] =
            v2; // here global id starts from 1
    }
    ...
    *(localFileStream) >> m_number_side;
    ...
    for (long iSide = 0; iSide < m_number_side; iSide++) {
        long v1, v2;
        *(localFileStream) >> v1 >> v2;
        m_local_side_id_to_global_id[iSide] =
            v2; // here global id starts from 1
    }
    ...
    double v2, h0;
    (*localFileStream) >> nrec >> v2 >> nspool >> m_number_layer >> kz >> h0 >>
        m_hs >> m_hc >> m_thetab >> m_thetaf >> ics;
    ...
    for (long iNode = 0; iNode < m_number_node; iNode++) {
        (*localFileStream) >> m_nodex[iNode] >> m_nodey[iNode] >> m_dp[iNode] >>
            m_kbp00[iNode];
    }
    // read in face nodes
    for (long iEle = 0; iEle < m_number_element; iEle++) {
...
// z is filled with 0
bool MDSCHISMMeshProvider::fillPointCoord2D(float*     a_pointCoord,
                                            const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::fillPointCoord2D(double*    a_pointCoord,
                                            const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
bool MDSCHISMMeshProvider::fillSideCenterCoord3D(float*     a_pointCoord,
                                                 const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::fillSideCenterCoord3D(double*    a_pointCoord,
                                                 const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::fillSideFaceCenterCoord3D(
...
// z is filled with 0
bool MDSCHISMMeshProvider::fillSideCenterCoord2D(float*     a_pointCoord,
                                                 const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
// z is filled with 0
bool MDSCHISMMeshProvider::fillSideCenterCoord2D(double*    a_pointCoord,
                                                 const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
bool MDSCHISMMeshProvider::fillEleCenterCoord3D(float*     a_pointCoord,
                                                const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::fillEleCenterCoord3D(double*    a_pointCoord,
                                                const int& a_timeStep) const {
...
// z is filled with 0
bool MDSCHISMMeshProvider::fillEleCenterCoord2D(float*     a_pointCoord,
                                                const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
// z is filled with 0
bool MDSCHISMMeshProvider::fillEleCenterCoord2D(double*    a_pointCoord,
                                                const int& a_timeStep) const {
    if (m_mesh_loaded == false) {
        return false;
    } else {
...
bool MDSCHISMMeshProvider::fillPointCoord3D(float*     a_pointCoord,
                                            const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::fillPointCoord3D(double*    a_pointCoord,
                                            const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::fillMeshElement(long* a_elementCache) const {
    ...
    for (long i = 0;
         i < (MeshConstants10::MAX_NUM_NODE_PER_CELL + 1) * m_number_element;
         i++) {
...
bool MDSCHISMMeshProvider::mesh_loaded() const { return m_mesh_loaded; }

// updates z coords at a timestep
bool MDSCHISMMeshProvider::zcoords2D(double*    a_zCachePtr,
                                     const int& a_timeStep) const {
    return true;
}

// updates z coords at a timestep
bool MDSCHISMMeshProvider::zcoords2D(float*     a_zCachePtr,
                                     const int& a_timeStep) const {
    return true;
}

bool MDSCHISMMeshProvider::zcoords3D(double*    a_zCachePtr,
                                     const int& a_timeStep) const {
    ...
    if (!(zVarPtr->is_valid())) {
        ...
        throw SCHISMFileException10("invlaid var " + MeshConstants10::ZCOORD +
                                    " for data file " + m_dataFilePtr->file());
    }
    ...
    if (!(zVarPtr->get(zPtr))) {
        throw SCHISMFileException10("fail to retrieve var " +
                                    MeshConstants10::ZCOORD +
    ...
    for (int iLayer = 0; iLayer < m_number_layer; iLayer++) {
        ...
        for (long iNode = 0; iNode < m_number_node; iNode++) {
            ...
            if (iLayer < (std::max(1, kbp00[iNode]) - 1)) {
                ...
                float temp =
                    zPtr[start_index + iLayer + 1 - std::max(1, kbp00[iNode])];
                ...
                } else // if not a valid z use computed one to finish mesh
                {
                    ...
                    float z = convertStoZ(sigma, surface, depth, m_hs, m_hc,
                                          m_thetab, m_thetaf);
                    *a_zCachePtr = z;
                }
                a_zCachePtr++;
...
bool MDSCHISMMeshProvider::zcoords3D(float*     a_zCachePtr,
                                     const int& a_timeStep) const {
...
double MDSCHISMMeshProvider::convertStoZ(const double& a_sigma,
    ...
    double csigma =
        (one - a_thetab) * sinh(a_thetaf * a_sigma) / sinh(a_thetaf) +
        a_thetab * (tanh(a_thetaf * (a_sigma + half)) - tanh(a_thetaf / two)) /
            (two * tanh(a_thetaf / two));
...
bool MDSCHISMMeshProvider::zcoords3D2(double*    a_zCachePtr,
                                      const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::zcoords3D2(float*     a_zCachePtr,
                                      const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::zSideCenter2D(double*    a_zCachePtr,
                                         const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::zSideCenter2D(float*     a_zCachePtr,
                                         const int& a_timeStep) const {
    for (long iNode = 0; iNode < m_number_side; iNode++) {
...
bool MDSCHISMMeshProvider::zSideCenter3D(double*    a_zCachePtr,
                                         const int& a_timeStep) const {
    double* zPtr = new double[m_number_layer * m_number_node];
    double* zPtrTemp = zPtr;
    zcoords3D2(zPtr, a_timeStep);
...
bool MDSCHISMMeshProvider::zSideCenter3D(float*     a_zCachePtr,
                                         const int& a_timeStep) const {
...
bool MDSCHISMMeshProvider::zEleCenter2D(double*    a_zCachePtr,
                                        const int& a_timeStep) const {
    for (long i = 0; i < m_number_element; i++) {
...
bool MDSCHISMMeshProvider::zEleCenter2D(float*     a_zCachePtr,
                                        const int& a_timeStep) const {
    for (long i = 0; i < m_number_element; i++) {
...
bool MDSCHISMMeshProvider::zEleCenter3D(double*    a_zCachePtr,
                                        const int& a_timeStep) const {
    double* zPtr = new double[m_number_layer * m_number_node];
    double* zPtrTemp = zPtr;
    zcoords3D2(zPtr, a_timeStep);
...
bool MDSCHISMMeshProvider::zEleCenter3D(float*     a_zCachePtr,
                                        const int& a_timeStep) const {
    float* zPtr = new float[m_number_layer * m_number_node];
    float* zPtrTemp = zPtr;
    zcoords3D2(zPtr, a_timeStep);
